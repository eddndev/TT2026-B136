...
\begin{table}[H]
  ...
  \begin{tabularx}{\linewidth}{@{}L{0.16\linewidth}L{0.22\linewidth}X L{0.16\linewidth}@{}}
    ...
    \addlinespace
    \textbf{Cincel Certificadora Digital S.A.P.I. de C.V.} & NOM-151, sellos de tiempo. & Primer PSC nativo de nube en México y América Latina; infraestructura AWS con modelo Zero Trust; API-first con sandbox gratuito; precios transparentes desde \$0.029 USD/transacción \cite{economia2026,comparasoftware2026}. & 2023 \\
    \bottomrule
  \end{tabularx}
\end{table}
...
\begin{itemize}
  ...
  \item \textbf{Disponibilidad de entorno de pruebas (sandbox).} Indispensable para implementar y validar el flujo criptográfico durante el ciclo de desarrollo sin incurrir en costos por cada transacción de prueba.
...
Del análisis del ecosistema de PSC conforme a estos criterios, \textbf{Cincel} es el proveedor seleccionado para el prototipo por las siguientes razones técnicas y operativas:

\begin{itemize}
  \item \textbf{Único PSC nativo de nube en México y América Latina acreditado por la Secretaría de Economía.} Su infraestructura está desplegada sobre Amazon Web Services (AWS) bajo el modelo de Seguridad de Confianza Cero (Zero Trust), lo que garantiza alta disponibilidad y resiliencia sin dependencia de centros de datos físicos propios del proveedor \cite{comparasoftware2026}.

  \item \textbf{Arquitectura API-first con sandbox gratuito.} Cincel expone su infraestructura a desarrolladores de forma inmediata, proporcionando colecciones públicas de Postman para pruebas de concepto instantáneas y un entorno de sandbox sin costo, lo que es determinante para la fase de desarrollo del prototipo \cite{comparasoftware2026}.

  \item \textbf{Modelo de consumo por transacción (pay-as-you-go).} Con un costo de aproximadamente \$0.029 USD por sello sin mínimos mensuales \cite{comparasoftware2026}, el esquema es compatible con el volumen reducido de transacciones de un prototipo de validación, a diferencia de los modelos de contrato corporativo de los PSC legados.

  \item \textbf{Cumplimiento nativo de RFC 3161 y NOM-151-SCFI-2016.} Los sellos digitales de tiempo emitidos por Cincel se adhieren estrictamente al protocolo RFC 3161 \cite{adams2001} y a los requisitos de sincronización metrológica con el CENAM, garantizando que las constancias generadas en el prototipo sean verificables de forma independiente ante cualquier instancia judicial.

  \item \textbf{Autorización de la Comisión Nacional Bancaria y de Valores (CNBV).} Cincel cuenta con autorización de la CNBV para prestar servicios al sector financiero mexicano \cite{comparasoftware2026}, lo que respalda adicionalmente la solidez de su infraestructura de seguridad para el manejo de documentos con valor probatorio.
\end{itemize}

Es importante señalar que la selección de Cincel como PSC del prototipo no implica exclusividad arquitectónica: el módulo de sellado de tiempo del sistema está diseñado para abstraer al proveedor detrás de una interfaz interna, de modo que el despacho pueda sustituir el PSC por cualquier otro del directorio de la Secretaría de Economía ---como SeguriData o CECOBAN--- sin modificar la lógica central del sistema, siempre que el proveedor alternativo implemente el protocolo RFC 3161. Esta decisión de diseño garantiza la soberanía tecnológica del despacho sobre su cadena de custodia digital.
