...
La \cref{tab:matriz-resumen} resume la distribución de las 367 funciones por crate y la \cref{tab:matriz-pruebas} presenta la matriz completa: una fila por archivo o módulo de prueba, agrupadas por crate. La columna \textbf{N} indica el número de funciones de prueba del archivo; la suma de la columna es 367. Las rutas se abrevian respecto de la raíz de cada crate: \texttt{src/} señala pruebas unitarias embebidas en el módulo (\texttt{\#[cfg(test)]}) y \texttt{tests/} señala binarios de prueba de integración. En la fila del stub del proveedor de sellado, la anotación $(+1)$ corresponde a la única prueba ignorada.

\begin{table}[H]
  \centering
  \caption{Distribución de las 367 funciones de prueba por crate. El conteo de integración de \texttt{infrastructure} incluye la prueba ignorada.}
...
El crate \texttt{web} aporta una prueba de contrato para \texttt{/healthz}; las rutas de negocio, sesiones, persistencia y RBAC quedan para los siguientes incrementos de integración.
...
\begin{table}[H]
  ...
  \begin{tabularx}{\linewidth}{@{}l R{2.6cm} R{2.2cm} R{2.4cm} X@{}}
    ...
    \texttt{bin} & 833 & 946 & 88.1\,\% & sin umbral (informativo) \\
    \midrule
    Total del workspace & 4\,808 & 5\,137 & 93.6\,\% & --- \\
    \bottomrule
  \end{tabularx}
\end{table}
...
El comando \texttt{auth calibrate} mide el costo real del hash de contraseñas con los parámetros fijos del prototipo (Argon2id con $m=256$ MiB, $t=4$, $p=1$; véase §3.11.5) contra la banda objetivo de 0.5--1.0 segundos por intento \cite{owasp_pwstorage2024,biryukov2021}. En el hardware de referencia AMD Ryzen 7 7730U la calibración de la demostración de cierre del 17 de agosto de 2026 reportó una \textbf{media de 607.8 ms sobre cinco corridas}, dentro de la banda objetivo, y fijó los parámetros en el adaptador. Si el hardware de despliegue difiere, la calibración debe repetirse; la suite conserva los bordes inclusivos de la banda (\texttt{band\_edges\_are\_inclusive}).
