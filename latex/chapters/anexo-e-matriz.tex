...
El workspace contiene \textbf{395 funciones de prueba}: 394 se ejecutan en verde en cada corrida y una se declara ignorada (\texttt{\#[ignore]}) porque es una prueba de humo contra el sandbox del proveedor externo de sellado de tiempo. El registro se completó, pero la API observada no ofrece estabilidad suficiente para una prueba determinista y el sandbox requiere pago; por ello, la prueba ignorada se conserva como referencia del adaptador y no se agenda como campaña de entrega. El resto del adaptador correspondiente se prueba contra un stub HTTP local, de modo que la suite regular no depende de ningún servicio de red ni incurre en costo externo.

Los nombres de las funciones de prueba son frases completas en \texttt{snake\_case} (en inglés, como todo identificador del código fuente) que describen el comportamiento verificado: por ejemplo, \texttt{a\_code\_is\_accepted\_once\_and\_rejected\_on\_reuse} documenta por sí misma la regla de un solo uso de los códigos de recuperación. Este estilo convierte al inventario de pruebas en la \emph{especificación ejecutable} del prototipo: la lectura de los nombres de una suite enumera las obligaciones de comportamiento del módulo correspondiente, y la corrida en verde certifica que todas se cumplen. Por esa razón la matriz no parafrasea cada prueba: agrupa las 395 funciones por crate y por archivo o módulo, indica su tipo y cita dos o tres nombres representativos por fila. El inventario íntegro se reproduce con \texttt{cargo test --workspace} sobre el repositorio.
...
La \cref{tab:matriz-resumen} resume la distribución de las 395 funciones por crate y la \cref{tab:matriz-pruebas} presenta la matriz completa: una fila por archivo o módulo de prueba, agrupadas por crate. La columna \textbf{N} indica el número de funciones de prueba del archivo; la suma de la columna es 395. Las rutas se abrevian respecto de la raíz de cada crate: \texttt{src/} señala pruebas unitarias embebidas en el módulo (\texttt{\#[cfg(test)]}) y \texttt{tests/} señala binarios de prueba de integración. En la fila del stub del proveedor de sellado, la anotación $(+1)$ corresponde a la única prueba ignorada.

\begin{table}[H]
  \centering
  \caption{Distribución de las 395 funciones de prueba por crate. El conteo de integración de \texttt{infrastructure} incluye la prueba ignorada.}
  ...
  \begin{tabularx}{\linewidth}{@{}X R{3.4cm} R{3.8cm} R{2.0cm}@{}}
    ...
    \midrule
    \texttt{domain} & 69 & 3 & 72 \\
    \texttt{application} & 39 & 46 & 85 \\
    \texttt{infrastructure} & 86 & 91 & 177 \\
    \texttt{bin} & 3 & 52 & 55 \\
    \texttt{web} & 1 & 5 & 6 \\
    \midrule
    Total & 198 & 197 & 395 \\
...
\begin{longtable}{@{}L{0.27\linewidth}C{0.05\linewidth}L{0.13\linewidth}L{0.45\linewidth}@{}}
  ...
  \bottomrule
  \endlastfoot
  \multicolumn{4}{@{}l@{}}{\textbf{Crate \texttt{domain}} --- 69 pruebas unitarias en \texttt{src/} y 3 de integración} \\
  \midrule
  ...
  \midrule
  \multicolumn{4}{@{}l@{}}{\textbf{Crate \texttt{application}} --- 39 unitarias en \texttt{src/} y 46 en \texttt{tests/}, con dobles de los puertos} \\
  \midrule
  ...
  \texttt{tests/document\_workflow.rs} & 4 & Integr. & \texttt{upload\_seal\_verify\_export\_and\_audit\_form\_one\_workflow}\newline
  ...
  \texttt{tests/identity\_workflow.rs} & 3 & Integr. & \texttt{owner\_bootstrap\_login\_totp\_authorization\_and\_logout\_form\_one\_flow}\newline
    \texttt{repeated\_bad\_passwords\_lock\_the\_login\_window} \\
  \midrule
  \multicolumn{4}{@{}l@{}}{\textbf{Crate \texttt{infrastructure}} --- 86 unitarias en \texttt{src/} y 91 de integración en \texttt{tests/} (una ignorada)} \\
  \midrule
  ...
  \texttt{tests/file\_document\_repository.rs} & 6 & Integr. & \texttt{insert\_and\_find\_round\_trip\_a\_pending\_record}\newline
  ...
  \midrule
  \multicolumn{4}{@{}l@{}}{\textbf{Crate \texttt{web}} --- 1 unitaria en \texttt{src/} y 5 de contrato en \texttt{tests/}} \\
  \midrule
  \texttt{src/lib.rs} & 1 & Unit. & \texttt{health\_endpoint\_reports\_ready} \\
  \texttt{tests/auth\_api.rs} & 2 & Integr. & \texttt{enrollment\_login\_totp\_me\_and\_logout\_have\_stable\_contracts}\newline
    \texttt{invalid\_login\_and\_client\_authorization\_have\_distinct\_statuses} \\
  \texttt{tests/document\_api.rs} & 3 & Integr. & \texttt{the\_document\_routes\_expose\_the\_complete\_workflow}\newline
    \texttt{upload\_requires\_bearer\_token\_and\_document\_name\_header}\newline
    \texttt{an\_unknown\_document\_maps\_to\_not\_found} \\
\end{longtable}
\endgroup

El crate \texttt{web} conserva la prueba de \texttt{/healthz}, tres pruebas del workflow documental protegido y dos del contrato de identidad. Los casos de uso se prueban en memoria y los adaptadores PostgreSQL/Redis contra procesos locales reales; \texttt{scripts/api-demo.sh} integra ambas capas con la TSA local.
...
La ejecución de \texttt{cargo test --workspace} reporta \textbf{394 pruebas en verde, 0 fallos y 1 ignorada}, distribuidas en 44 binarios de prueba. Sobre un árbol ya compilado, la suite completa termina en segundos; las pruebas más costosas son las que ejecutan subprocesos reales de \texttt{openssl} y del binario propio. Este costo bajo es deliberado: los instantes de evaluación de los certificados se derivan de la ventana de validez parseada del propio certificado y no de esperas de reloj, de modo que la suite es determinista y puede correrse en cada cambio sin fricción.
...
\begin{table}[H]
  ...
  \begin{tabularx}{\linewidth}{@{}l R{2.6cm} R{2.2cm} R{2.4cm} X@{}}
    ...
    \midrule
    \texttt{domain} & 973 & 1\,013 & 96.1\,\% & $\geq$ 90\,\% \\
    \texttt{application} & 1\,749 & 1\,885 & 92.8\,\% & $\geq$ 90\,\% \\
    \texttt{infrastructure} & 2\,257 & 2\,436 & 92.7\,\% & $\geq$ 90\,\% \\
    \texttt{bin} & 834 & 1\,034 & 80.7\,\% & sin umbral (informativo) \\
    \texttt{web} & 319 & 436 & 73.2\,\% & sin umbral (informativo) \\
    \midrule
    Total del workspace & 6\,132 & 6\,804 & 90.1\,\% & --- \\
    \bottomrule
  \end{tabularx}
\end{table}
...
El comando \texttt{auth calibrate} mide el costo real del hash de contraseñas con los parámetros fijos del prototipo (Argon2id con $m=256$ MiB, $t=2$, $p=1$; véase §3.11.5) contra la banda objetivo de 0.5--1.0 segundos por intento \cite{owasp_pwstorage2024,biryukov2021}. En el hardware de referencia AMD Ryzen 7 7730U la calibración de la demostración de cierre del 17 de agosto de 2026 reportó una \textbf{media de 529.4 ms sobre cinco corridas}, dentro de la banda objetivo, y fijó los parámetros en el adaptador. Si el hardware de despliegue difiere, la calibración debe repetirse; la suite conserva los bordes inclusivos de la banda (\texttt{band\_edges\_are\_inclusive}).

\subsection{Transcripción de la demostración}\label{sec:matriz-demo}

La evidencia de ejecución de cara al usuario final comprende dos guiones. \texttt{scripts/demo.sh} recorre por CLI el ciclo de vida completo de la evidencia, autenticación y bitácora; su transcripción íntegra, capturada sobre OpenSSL 3.5.7, está versionada en \texttt{docs/demo-transcript.md}. \texttt{scripts/api-demo.sh} genera una PKI temporal, arranca la aplicación sobre un puerto efímero y recorre carga, persistencia cifrada, sellado, verificación, exportación y auditoría a través de HTTP; después comprueba el ZIP con \texttt{openssl} y \texttt{unzip}. Cada condición de ambos guiones es un punto de verificación real y cualquiera aborta ante un resultado inesperado. El Anexo~\ref{anexo:verificacion} reproduce y explica la porción de la primera transcripción dedicada a la verificación independiente del paquete de evidencia.

\subsection{Re-ejecución continua}\label{sec:matriz-ci}

Toda la evidencia anterior se regenera automáticamente en cada cambio: el flujo de integración continua del repositorio (\texttt{.github/workflows/ci.yml}) ejecuta siete jobs sobre cada push y cada pull request, y el fallo de cualquiera de ellos ---salvo el job \texttt{msrv}, hoy informativo--- bloquea la integración del cambio. La \cref{tab:matriz-ci} los enumera. De este modo, la matriz de este anexo no describe una fotografía única sino un invariante mantenido: las 394 pruebas en verde, los umbrales de cobertura y las fitness functions se verifican en cada modificación del código.
