...
Para impedir que un firmante niegue haber suscrito un contrato, el sistema genera evidencia criptográfica verificable de forma independiente sobre la autoría y el momento exacto de cada firma, proveyendo así no repudio en el flujo contractual del despacho.
...
\begin{itemize}
  ...
  \item Implementar el servicio de no repudio para el flujo de firma de contratos mediante firma digital (véase §2.3.3) con certificados X.509 (véase §2.3.3.3) y sellado de tiempo (véase §2.3.3.4) emitido por un Prestador de Servicios de Certificación autorizado conforme a la NOM-151-SCFI-2016 (véase §2.2.7) y al protocolo RFC (Request for Comments) 3161, generando evidencia criptográfica verificable sobre la autoría y el momento de cada firma.
...
\begin{table}[H]
  ...
  \begin{tabularx}{\linewidth}{@{}l L{0.30\linewidth} X@{}}
    ...
    \midrule
    OE-1 & Análisis y diseño funcional/técnico & Catálogo completo de 18 casos de uso derivados de las 7 agrupaciones de historias de usuario; arquitectura hexagonal documentada con al menos tres puertos (persistencia, criptografía, TSA); modelo de datos híbrido (relacional y volátil) definido con esquemas normalizados. \\
    ...
    OE-3 & No repudio con sello RFC 3161 & Firma digital RSA-3072 con SHA-256 verificable de forma independiente sobre el 100 \% de los contratos firmados; sello de tiempo RFC 3161 vinculado al hash del documento; tasa de verificación exitosa $\geq$ 99 \% sobre sellos emitidos por el sandbox de Cincel. \\
    ...
    OE-5 & Autenticación diferenciada por rol & Argon2id calibrado a 300--500 ms por intento en el hardware de despliegue (m=46 MiB, t=1, p=1); MFA basado en TOTP (RFC 6238 \cite{mraihi2011}) con ventana de tolerancia $\pm$1 período; control de acceso RBAC con cuatro roles diferenciados (Owner, Litigante, Paralegal, Cliente). \\
...
No repudio con evidencia temporal. Implementación de firma digital mediante certificados X.509 y sellado de tiempo conforme al protocolo RFC 3161, vinculando criptográficamente la autoría del firmante con el contenido del contrato y una marca temporal emitida por un PSC autorizado.
...
Autenticación robusta y gestión segura de credenciales. Control de acceso mediante certificados digitales para socios y autenticación por múltiples factores (MFA) para clientes, complementado con almacenamiento de contraseñas bajo funciones hash resistentes (Argon2 o bcrypt), mitigando riesgos de suplantación de identidad.
...
\begin{itemize}
  ...
  \item La firma digital emitida por el prototipo es firma electrónica simple en los términos del artículo 89 del Código de Comercio. Al no provenir de un Prestador de Servicios de Certificación autorizado, no alcanza la presunción de firma avanzada establecida en el artículo 97 del mismo ordenamiento. El respaldo probatorio se sustenta en el sello de tiempo conforme a la NOM-151-SCFI-2016.

  \item La integración con el PSC Cincel se realiza contra su entorno de sandbox para los fines del Trabajo Terminal. El despliegue en producción requeriría contratación comercial de la API y queda fuera del alcance.
\end{itemize}
...
\begin{itemize}
  \item El servicio de sellado de tiempo de Cincel (API v3, entorno sandbox) se mantiene disponible durante el ciclo de desarrollo y pruebas del prototipo.

  \item El hardware de despliegue dispone de al menos 46 MiB de memoria por proceso de autenticación, condición necesaria para ejecutar Argon2id con los parámetros seleccionados sin degradar el tiempo de respuesta del inicio de sesión.

  \item Existe conectividad estable a internet para consultar listas de revocación (CRL) y servicios OCSP de los PSCs mexicanos, así como la autoridad de sellado de tiempo de Cincel.
