...
\subsection{Estado de implementación evaluado}\label{sec:alcance-implementado}

Los alcances y criterios anteriores describen el producto propuesto. El corte
evaluado del 12 de septiembre de 2026 comprende la CLI, el núcleo criptográfico
y una API HTTP con autenticación por contraseña y segundo factor, sesiones
opacas revocables, expedientes, asignaciones y operaciones documentales
protegidas por pertenencia vigente. La PKI interviene en la firma y verificación
de evidencia; no se ha implementado un login mediante certificado de usuario.
PostgreSQL conserva documentos cifrados y auditoría bajo una misma frontera
transaccional; Redis mantiene el estado efímero de autenticación.

La interfaz de navegador sigue sin implementar los flujos del producto.
El historial documental, participantes procesales, audiencias, plazos y calendario,
así como las pruebas de usabilidad, son actividades pendientes. Los
\cref{cap:implementacion,cap:pruebas,cap:conclusiones} delimitan qué objetivos
cuentan con evidencia técnica y cuáles requieren desarrollo o evaluación
posterior; no se deduce su cumplimiento del número total de pruebas.
