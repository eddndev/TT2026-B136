...
Cada paquete de evidencia exportado por el prototipo (mediante la CLI o la descarga HTTP autorizada por expediente) viaja con un instructivo \texttt{INSTRUCCIONES.md} que permite a un tercero ---un perito, la contraparte procesal o un tribunal--- verificar el documento, su firma, el estado del certificado y el sello de tiempo usando únicamente la herramienta de línea de comandos \texttt{openssl}, sin el software que generó el paquete. El instructivo se genera a partir de la plantilla versionada \texttt{crates/application/src/evidence/\allowbreak INSTRUCCIONES.template.md}, cuyos marcadores se rellenan al momento de exportar con los datos concretos del paquete (\cref{tab:verificacion-marcadores}). Este anexo reproduce esa guía paso a paso y explica qué demuestra cada comando; con ello se materializa el criterio de verificabilidad independiente del objetivo OE-3. Las salidas mostradas son transcripciones históricas de \rutarepo{docs/demo-transcript.md}. El ensayo del 12 de septiembre de 2026 añadió la comparación de paquetes obtenidos desde dos servidores y después de migrar y restaurar los documentos, según el \cref{cap:pruebas}.
...
\begin{table}[H]
  ...
  \begin{tabularx}{\linewidth}{@{}l X@{}}
    ...
    \texttt{crl.pem} & La lista de revocación capturada con la evidencia; su vigencia se evalúa según sus fechas. \\
    \texttt{tsa-chain.pem} & La cadena de certificados de la autoridad de sellado capturada con el token, cuando se suministró. \\
...
\textbf{Qué demuestra.} Que el token contiene una aseveración temporal firmada sobre el resumen del documento y que su firma se verifica bajo el ancla suministrada. Modificar el documento rompe la correspondencia con ese resumen. La confianza en la fecha depende de la autoridad y de su reloj; una TSA local controlada por el operador no aporta por sí misma independencia ni una constancia de un prestador autorizado.
...
\begin{table}[H]
  ...
  \begin{tabularx}{\linewidth}{@{}l l l X@{}}
    ...
    4 & \texttt{openssl ts -verify} & \texttt{Verification: OK} & Evidencia temporal: la TSA firmó una aseveración sobre el resumen y el instante indicado, bajo el ancla suministrada. \\
    \bottomrule
  \end{tabularx}
\end{table}
...
La CLI y la descarga HTTP tienen controles diferentes. El comando
\texttt{package export} comprueba integralmente los artefactos antes de escribir
el ZIP. La descarga HTTP descifra el registro, conserva el material capturado y
confirma autorización y auditoría antes de devolverlo; esa exportación no ejecuta
una nueva validación de firma, certificado y TSA. La operación HTTP de verificación
está separada. Por ello, disponer de un ZIP no reemplaza ejecutar las comprobaciones
independientes ni establecer la confianza en sus anclas.

La suite extrae paquetes reales y ejecuta los comandos del instructivo, incluida
la detección de un byte alterado, en
\rutarepo{crates/bin/tests/package_cli.rs}. El ensayo de migración y restauración
compara los ZIP byte por byte y vuelve a comprobar firma, certificado, CRL y sello
con OpenSSL. Su éxito demuestra interoperabilidad y conservación en las condiciones
del ensayo, no que toda verificación futura deba producir el mismo resultado.

\section{Evidencia histórica y momento de evaluación}\label{sec:verificacion-historica}

El registro HTTP conserva certificados, CRL y token capturados al sellar.
Exportar no actualiza la CRL; su vigencia depende de sus fechas de emisión y
expiración.

\texttt{openssl verify} evalúa por defecto al ejecutarse. El importador comprueba
cadena y CRL del firmante en el instante autenticado del sello, sin regenerar
evidencia. La TSA conserva la política actual de OpenSSL y puede ser rechazada
si expiró. Deben registrarse instante y política de evaluación; preservar bytes
no garantiza aceptación posterior.
