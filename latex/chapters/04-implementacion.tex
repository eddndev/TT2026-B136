...
El adaptador restringe primero al expediente autorizado, aplica los filtros y ordena por UUID antes de paginar. Obtiene como máximo una fila adicional para calcular \texttt{has\_more}; la respuesta contiene \texttt{documents} y ese indicador, sin un conteo global. El orden es estable para un conjunto sin modificaciones; varias peticiones no comparten una instantánea, por lo que altas concurrentes pueden modificar la composición de las páginas. Un expediente visible sin coincidencias devuelve una página vacía.

La proyección SQL selecciona únicamente identidad, versión, nombre, digest y presencia de evidencia; no carga ni descifra el vault y no deserializa firmas, certificados o tokens. El detalle aplica la misma proyección a la asociación exacta expediente/documento. Por ello, obtener metadatos no constituye una verificación criptográfica de integridad: el usuario solicita esa comprobación mediante la operación específica de verificación, que presenta sus cuatro componentes.
