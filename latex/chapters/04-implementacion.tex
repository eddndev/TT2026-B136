...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\linewidth}{@{}L{7.2cm} X@{}}
    ...
    \midrule
    \texttt{serve -{}-signer-cert <CERT> -{}-signer-key <KEY> [-{}-bind <ADDR>] [-{}-data-dir <DIR>]} & Inicia la aplicación HTTP local con persistencia cifrada y TSA OpenSSL. \\
    \midrule
...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\linewidth}{@{}L{3.4cm} L{5.2cm} >{\raggedright\arraybackslash}X@{}}
    ...
    \midrule
    \texttt{serve} & \texttt{LocalDocumentWorkflow} & repositorio JSON local, bitácora, cifrado, firma, validación, TSA local y ZIP \\
...
\section{Aplicación HTTP y persistencia local}\label{sec:impl-http}

La superficie HTTP se construyó como un adaptador driving en \texttt{crates/web}: sus handlers convierten cabeceras, UUID, cuerpos binarios y respuestas JSON, pero solo conocen el puerto \texttt{Document\allowbreak Workflow} de \texttt{application}. La implementación \texttt{LocalDocument\allowbreak Workflow} orquesta los puertos ya probados de hash, cifrado, envoltura de claves, firma, validación X.509, sellado RFC 3161, verificación, archivo ZIP, reloj y auditoría. La raíz de composición del comando \texttt{serve} selecciona explícitamente \texttt{LocalOpensslTsa}; por ello, el camino de ejecución de la aplicación no lee la configuración de Cincel ni puede degradarse silenciosamente hacia un proveedor externo.

El flujo comienza con \texttt{POST /api/v1/documents}. El cuerpo es el documento binario, limitado a 16~MiB; las cabeceras \texttt{X-Document-Name} y \texttt{X-Actor} aportan el nombre seguro para archivo y la etiqueta de auditoría. La aplicación crea un UUID y la versión uno, calcula SHA-256, cifra mediante el formato \texttt{DVLT1} ligado al par identidad--versión y persiste el registro. \texttt{X-Actor} no es una credencial ni una autorización: únicamente atribuye el evento, por lo que la escucha predeterminada se limita a \texttt{127.0.0.1} y la exposición multiusuario queda condicionada a sesiones y RBAC.

El repositorio \texttt{FileDocument\allowbreak Repository} escribe un JSON por UUID bajo \texttt{runtime-data/\allowbreak documents}; los campos binarios se codifican en base64 y el contenido documental se conserva exclusivamente dentro del paquete cifrado. Cada actualización se prepara en un archivo temporal y se publica por reemplazo atómico, mientras un lock advisory evita carreras entre escritores. La bitácora encadenada se mantiene en \texttt{runtime-data/\allowbreak audit.jsonl}. Esta persistencia permite demostrar reinicio y consistencia local sin afirmar equivalencia con PostgreSQL y Redis, que siguen siendo los adaptadores previstos para producción.

\begin{table}[H]
  \centering
  \caption{Contrato de la aplicación HTTP local.}
  ...
  \begin{tabularx}{\linewidth}{@{}L{6.2cm} X@{}}
    ...
    \texttt{POST /api/v1/documents} & Carga, cifra y persiste la versión inicial; responde \texttt{201}. \\
    \texttt{POST /api/v1/\allowbreak documents/<id>/seal} & Firma el digest, obtiene un token de la TSA local, lo verifica y almacena evidencia inmutable. \\
    \texttt{POST /api/v1/\allowbreak documents/<id>/verify} & Descifra y reporta integridad, firma, certificado/CRL, sello y veredicto. \\
    \texttt{GET /api/v1/\allowbreak documents/<id>/evidence} & Exporta el ZIP interoperable tras una verificación satisfactoria. \\
    \texttt{GET /api/v1/audit/verify} & Verifica el encadenamiento y reporta el primer índice roto. \\
    \bottomrule
  \end{tabularx}
\end{table}

Sellar es una transición de una sola vez: antes de guardar, el workflow descifra el paquete, comprueba que coincide con el digest almacenado, firma, verifica que la llave corresponde al certificado, solicita el token local y lo verifica contra la cadena de la TSA. Verificar reutiliza el reporte de cuatro componentes del caso de uso existente. Exportar vuelve a verificar y materializa el texto claro solamente en la respuesta ZIP, junto con firma, certificado, CA, CRL, token y cadena de la TSA. Los errores se traducen a una envoltura JSON estable: entrada inválida en \texttt{400} o \texttt{422}, ausencia en \texttt{404}, transición incompatible en \texttt{409} y fallos internos redactados en \texttt{500}.

El guion \rutarepo{scripts/api-demo.sh} genera una PKI temporal, desactiva expresamente las variables de Cincel, inicia \texttt{serve} sobre un puerto efímero y recorre carga, comprobación de ausencia del texto claro en el repositorio, sellado, verificación, descarga del ZIP y auditoría. Después extrae la evidencia y verifica con herramientas ajenas a la aplicación la igualdad del documento, la firma, el certificado con CRL y el sello RFC 3161. El contrato operativo completo se entrega en \rutarepo{docs/http-api.md} y la decisión arquitectónica en \rutarepo{docs/adr/0011-local-document-workflow.md}.
...
\begin{itemize}
  ...
  \item \textbf{Superficie driving y persistencia.} De los tres adaptadores driving previstos (\cref{sec:vista-componentes}), el prototipo entrega la línea de comandos y una rebanada HTTP completa del flujo de evidencia; el frontend Astro sigue como andamiaje mínimo intencional. La persistencia PostgreSQL y Redis del diseño (\cref{sec:bd-hibrida}) aún no se ejercita: los documentos se almacenan cifrados en JSON con reemplazo atómico y la bitácora en JSON Lines encadenado. El repositorio local demuestra el puerto y la continuidad entre reinicios, pero no se presenta como almacenamiento de producción.
