...
\begin{table}[H]
  ...
  \begin{tabularx}{\linewidth}{@{}l X R{3.2cm} R{3.2cm}@{}}
    ...
    \texttt{application} & Casos de uso genéricos sobre los puertos & 2\,969 & 1\,694 \\
    \texttt{infrastructure} & Adaptadores driven concretos & 3\,770 & 2\,570 \\
    \texttt{web} & Adaptador driving HTTP y DTO de contrato & 367 & 212 \\
    \texttt{bin} & Raíz de composición, CLI y servidor HTTP & 1\,892 & 2\,835 \\
    \bottomrule
  \end{tabularx}
\end{table}
...
\begin{table}[H]
  ...
  \begin{tabularx}{\linewidth}{@{}l X@{}}
    ...
    \texttt{DATABASE\_URL} & Cadena de conexión PostgreSQL para usuarios y migraciones. \\
...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\linewidth}{@{}L{7.2cm} X@{}}
    ...
    \midrule
    \texttt{serve -{}-signer-cert <CERT> -{}-signer-key <KEY> [-{}-bind <ADDR>] [-{}-data-dir <DIR>]} & Inicia la aplicación HTTP autenticada con PostgreSQL, Redis, documentos cifrados y TSA OpenSSL. \\
    \midrule
...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\linewidth}{@{}L{3.4cm} L{5.2cm} >{\raggedright\arraybackslash}X@{}}
    ...
    \midrule
    \texttt{serve} & \texttt{IdentityService} + \texttt{LocalDocumentWorkflow} & PostgreSQL, Redis, Argon2id, TOTP, repositorio JSON cifrado, bitácora, firma, TSA local y ZIP \\
    ...
    \texttt{auth calibrate}, \texttt{hash-password}, \texttt{verify-password} & \texttt{CalibratePassword\allowbreak Hashing}, \texttt{HashPassword}, \texttt{VerifyPassword} & \texttt{Argon2idHasher} \\
...
\section{Aplicación HTTP autenticada y persistencia híbrida}\label{sec:impl-http}

La superficie HTTP se construyó como un adaptador driving en \texttt{crates/web}: sus handlers convierten cabeceras, UUID, cuerpos binarios y respuestas JSON, pero solo conocen los puertos \texttt{IdentityWorkflow} y \texttt{Document\allowbreak Workflow} de \texttt{application}. La implementación \texttt{IdentityService} orquesta usuarios, credenciales, segundo factor, sesiones y auditoría; \texttt{LocalDocument\allowbreak Workflow} orquesta hash, cifrado, envoltura de claves, firma, validación X.509, sellado RFC 3161, verificación y archivo ZIP. Como ambos conjuntos de adaptadores realizan trabajo síncrono, los handlers trasladan cada caso de uso al pool bloqueante de Tokio: esta separación evita el fallo de runtime anidado que una primera prueba integral detectó al invocar el cliente PostgreSQL desde un worker asíncrono. La raíz de composición selecciona explícitamente \texttt{LocalOpensslTsa}; el camino de ejecución no lee la configuración de Cincel ni puede degradarse silenciosamente hacia un proveedor externo.

El flujo de identidad comienza en \texttt{POST /api/v1/auth/bootstrap}: solo funciona mientras PostgreSQL no contenga usuarios y una transacción con bloqueo exclusivo de la tabla hace atómica la creación del primer Owner. El correo se normaliza, la contraseña se almacena como PHC Argon2id y el enrolamiento devuelve una sola vez el secreto TOTP y ocho códigos de recuperación. \texttt{POST /api/v1/auth/login} verifica la contraseña y crea en Redis un desafío de cinco minutos; TOTP o un código de recuperación consumido completan la autenticación y emiten un bearer token opaco de 256 bits con TTL de 24 horas. Redis nunca usa el token en claro como clave: deriva su índice con SHA-256. Logout elimina ese índice de inmediato. Cinco contraseñas rechazadas bloquean por 15 minutos la clave derivada del correo, sin distinguir por error si la cuenta existe; un segundo factor rechazado consume su desafío.

La tabla \texttt{users} de PostgreSQL conserva UUID, correo único, PHC, rol, estado activo, secreto TOTP protegido, códigos de recuperación hasheados y una revisión para actualización optimista. \texttt{AesGcmSecretProtector} cifra el secreto TOTP con la misma KEK de runtime y lo liga al UUID mediante datos autenticados. Cada petición protegida resuelve la sesión en Redis y relee rol y estado desde PostgreSQL, por lo que una desactivación o cambio de rol no queda congelado durante las 24 horas del token. Los cuatro roles son Owner, Litigante, Paralegal y Cliente. El Cliente permanece sin permisos documentales hasta que la pertenencia a casos sea un atributo durable; así se evita convertir el conocimiento de un UUID en autorización transversal.

El flujo documental comienza con \texttt{POST /api/v1/documents}. El cuerpo binario se limita a 16~MiB y \texttt{X-Document-Name} aporta el nombre seguro. \texttt{X-Actor} fue eliminado: el actor de auditoría procede exclusivamente del bearer token y la acción se contrasta con la matriz RBAC antes de invocar el workflow. La aplicación crea un UUID y la versión uno, calcula SHA-256, cifra mediante \texttt{DVLT1} ligado al par identidad--versión y persiste el registro. \texttt{FileDocument\allowbreak Repository} escribe un JSON cifrado por UUID bajo \texttt{runtime-data/\allowbreak documents}; la bitácora encadenada se mantiene en \texttt{runtime-data/\allowbreak audit.jsonl}. Usuarios y autenticación ya ejercitan PostgreSQL y Redis; la migración relacional de documentos y auditoría permanece como límite explícito.
...
\begin{table}[H]
  ...
  \begin{tabularx}{\linewidth}{@{}L{6.2cm} X@{}}
    ...
    \midrule
    \texttt{POST /api/v1/auth/bootstrap} & Crea atómicamente el primer Owner y entrega material de enrolamiento; responde \texttt{201}. \\
    \texttt{POST /api/v1/auth/login} & Verifica la contraseña y entrega un desafío MFA de cinco minutos. \\
    \texttt{POST /api/v1/auth/mfa/totp} & Verifica y reclama un TOTP; crea una sesión revocable. \\
    \texttt{POST /api/v1/auth/mfa/recovery} & Consume un código recovery con actualización optimista; crea una sesión. \\
    \texttt{GET /api/v1/auth/me} & Devuelve identidad y rol vigentes de la sesión. \\
    \texttt{POST /api/v1/auth/logout} & Revoca la sesión; responde \texttt{204}. \\
    \texttt{POST /api/v1/users} & Permite al Owner crear otra cuenta y enrolar su segundo factor. \\
    \bottomrule
  \end{tabularx}
\end{table}
...
\begin{table}[H]
  \centering
  \caption{Contrato HTTP documental protegido por RBAC.}
  \label{tab:impl-http-rutas}
  \begin{tabularx}{\linewidth}{@{}L{6.2cm} X@{}}
    \toprule
    \textbf{Ruta} & \textbf{Responsabilidad} \\
    \midrule
    \texttt{GET /healthz} & Diagnóstico del proceso. \\
    \texttt{POST /api/v1/documents} & Owner, Litigante o Paralegal cargan, cifran y persisten la versión inicial; responde \texttt{201}. \\
    \texttt{POST /api/v1/\allowbreak documents/<id>/seal} & Owner o Litigante firman el digest y almacenan evidencia inmutable con sello local verificado. \\
    \texttt{POST /api/v1/\allowbreak documents/<id>/verify} & Owner, Litigante o Paralegal verifican los cuatro componentes. \\
    \texttt{GET /api/v1/\allowbreak documents/<id>/evidence} & Owner, Litigante o Paralegal exportan el ZIP interoperable. \\
    \texttt{GET /api/v1/audit/verify} & Solo Owner verifica la cadena completa y el primer índice roto. \\
    \bottomrule
  \end{tabularx}
\end{table}

Sellar es una transición de una sola vez: antes de guardar, el workflow descifra el paquete, comprueba que coincide con el digest almacenado, firma, verifica que la llave corresponde al certificado, solicita el token local y lo verifica contra la cadena de la TSA. Verificar reutiliza el reporte de cuatro componentes. Exportar vuelve a verificar y materializa el texto claro solamente en la respuesta ZIP. Los errores se traducen a una envoltura JSON estable: credenciales, MFA o sesión en \texttt{401}; permiso insuficiente en \texttt{403}; entrada inválida en \texttt{400} o \texttt{422}; ausencia en \texttt{404}; conflicto en \texttt{409}; bloqueo en \texttt{429}; y fallos internos redactados en \texttt{500}.

El guion \rutarepo{scripts/api-demo.sh} levanta PostgreSQL y Redis en puertos efímeros, genera una PKI temporal, desactiva Cincel e inicia \texttt{serve}. Crea un Owner y un Paralegal, calcula TOTP desde un cliente independiente, comprueba un \texttt{401} sin sesión y un \texttt{403} al intentar sellar como Paralegal, recorre el workflow documental, revoca por logout y demuestra que un recovery code se acepta una sola vez. También busca la contraseña en PostgreSQL, el token en las claves Redis y el texto claro en documentos persistidos. Finalmente verifica el ZIP con OpenSSL. El contrato está en \rutarepo{docs/http-api.md}; las decisiones, en \rutarepo{docs/adr/0011-local-document-workflow.md} y \rutarepo{docs/adr/0012-revocable-sessions-and-rbac.md}.
...
\begin{itemize}
  ...
  \item \textbf{Superficie driving y persistencia.} De los tres adaptadores driving previstos (\cref{sec:vista-componentes}), el prototipo entrega la línea de comandos y una rebanada HTTP multiusuario; el frontend Astro sigue como andamiaje mínimo intencional. PostgreSQL ya persiste usuarios y Redis el estado efímero de autenticación. Los documentos continúan cifrados en JSON y la bitácora en JSON Lines encadenado; esa porción local demuestra los puertos y la continuidad entre reinicios, pero no se presenta como almacenamiento transaccional de producción.
  \item \textbf{Sesiones opacas en lugar de JWT.} El diseño nombró JWT para la frontera autenticada. La implementación usa tokens aleatorios opacos porque la revocación inmediata ya exige consultar Redis y porque duplicar el rol dentro de un JWT permitiría que la autorización quedara obsoleta. Cada petición recarga estado activo y rol desde PostgreSQL; la sustitución permanece detrás del puerto \texttt{SessionStore} (\rutarepo{docs/adr/0012}).
  \item \textbf{Clientes de datos síncronos.} El diseño eligió \texttt{sqlx} y \texttt{fred}; el corte compatible con Rust 1.78 usa los clientes síncronos \texttt{postgres} y \texttt{redis}, fijados a versiones compatibles con la MSRV. La frontera HTTP aísla los casos de uso en el pool bloqueante de Tokio. Antes de despliegue público deberán evaluarse pooling, TLS interno y clientes asíncronos.
  ...
  \item \textbf{Alcance por recurso diferido.} El RBAC global está implementado, incluido el reclamo atómico de TOTP y el límite de cinco contraseñas fallidas en 15 minutos. El rol Cliente se mantiene sin acceso documental hasta persistir la relación entre usuario, expediente y caso; conceder acceso por UUID sin ese atributo violaría el aislamiento entre clientes.
\end{itemize}
