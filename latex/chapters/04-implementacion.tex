...
\begin{table}[H]
  ...
  \begin{tabularx}{\linewidth}{@{}l X R{3.2cm} R{3.2cm}@{}}
    ...
    \texttt{web} & Adaptador driving HTTP (\texttt{/healthz}) & 41 & 0 \\
...
\begin{itemize}
  ...
  \item \textbf{Superficie driving y persistencia.} De los tres adaptadores driving previstos (\cref{sec:vista-componentes}), el prototipo entrega primero el de línea de comandos y un router HTTP con \texttt{/healthz}; el frontend Astro es un andamiaje mínimo intencional. Análogamente, la persistencia PostgreSQL y Redis del diseño (\cref{sec:bd-hibrida}) no se ejercita aún: la bitácora persiste en un archivo JSON Lines con encadenamiento criptográfico y \texttt{DATABASE\_URL} queda reservada; los casos de uso, sesiones y repositorios se agregarán sobre el adaptador HTTP.
