...
\begin{itemize}
  ...
  \item \textbf{Iteración 9 (Semana 9) - Persistencia, Control de Acceso y Gestión Procesal:} Implementación de la persistencia híbrida PostgreSQL/Redis y del control de acceso mediante sesiones opacas revocables, roles (RBAC) y asignación al expediente. Desarrollo de la gestión procesal y documental —módulos CRUD para expedientes, casos, contratos y documentos— integrando los servicios criptográficos ya probados. Desarrollo de una interfaz web mínima.
...
\begin{table}[H]
  ...
  \begin{tabularx}{\linewidth}{@{}>{\bfseries}l X@{}}
    ...
    Actor principal & Litigante \\
    Actores secundarios & --- \\
    Precondiciones & El expediente tiene perfil completo y estado administrativo activo, y su etapa registrada admite el avance ordinario solicitado (Investigación o Intermedia). El Litigante tiene asignación vigente. \\
    Flujo principal & 1. El Litigante solicita el avance de etapa. 2. El sistema pide el soporte habilitante: escrito de acusación para Investigación → Intermedia; auto de apertura para Intermedia → Juicio Oral. 3. El Litigante selecciona una versión documental existente o carga un documento nuevo y selecciona su versión confirmada. 4. Si hay carga, el sistema la almacena con integridad y confidencialidad; después verifica la admisión del soporte para la transición. 5. El sistema registra inmutablemente la transición, la fecha de registro, el usuario y la identidad, versión y resumen criptográfico del soporte, conservando su evidencia histórica. 6. El sistema habilita los módulos correspondientes a la nueva etapa. \\
    Flujos alternos & 2a. No se selecciona un soporte habilitante → el sistema rechaza la transición e indica el documento requerido. 3a. El formato del soporte no es admitido → el sistema rechaza su admisión a la transición; una carga ya confirmada se conserva. \\
    Postcondiciones & La nueva etapa y su soporte exacto quedan registrados en el historial inmutable del expediente, sin reversión de la transición ni sustitución de la evidencia histórica. \\
...
\begin{figure}[H]
  \centering
  \resizebox{\linewidth}{!}{\input{figures/etapas}}
...
\begin{table}[H]
  ...
  \begin{tabularx}{\linewidth}{@{}L{3.2cm}X@{}}
    ...
    \textbf{Actores involucrados} & Litigante, Paralegal; sistema de almacenamiento PostgreSQL (registro de auditoría). \\
...
\begin{table}[H]
  ...
  \begin{tabularx}{\linewidth}{@{}L{3.2cm}X@{}}
    ...
    \textbf{Salida} & Registro append-only en base de datos PostgreSQL con los campos: \texttt{user\_id}, \texttt{action\_type}, \texttt{resource\_id}, \texttt{timestamp}, \texttt{ip\_address}. \\
    \textbf{Actores involucrados} & Todos los roles del sistema; motor PostgreSQL de auditoría. \\
...
\begin{table}[H]
  ...
  \begin{tabularx}{\linewidth}{@{}L{3.2cm}X@{}}
    ...
    \textbf{Actores involucrados} & Cualquier usuario o proceso del sistema; motor de base de datos PostgreSQL. \\
...
\begin{table}[H]
  ...
  \begin{tabularx}{\linewidth}{@{}L{0.22\linewidth}L{0.20\linewidth}L{0.14\linewidth}X@{}}
    ...
    Base de datos & PostgreSQL + Redis (open source) & \$0.00 MXN & Motores de base de datos open source desplegados en el VPS del equipo. \\
...
\begin{table}[H]
  ...
  \begin{tabularx}{\linewidth}{@{}>{\bfseries}l X@{}}
    ...
    VME & \$6,562.50 MXN \\
    Estado & Materializado. El registro se completó, pero la API observada no ofreció estabilidad suficiente para una campaña repetible y el sandbox requiere pago. \\
...
\begin{table}[H]
  ...
  \begin{tabularx}{\linewidth}{@{}>{\bfseries}l X@{}}
    ...
    Mitigación & Diseñar la capa de persistencia PostgreSQL para prohibir operaciones UPDATE o DELETE del rol operativo en las bitácoras, forzando un esquema estrictamente append-only desde el inicio. \\
...
\begin{table}[H]
  ...
  \begin{tabularx}{\linewidth}{@{}>{\bfseries}l X@{}}
    ...
    Impacto & 2 -- Medio (20 h) \\
    VME & \$3,125.00 MXN \\
    Estado & Materializado parcialmente como degradación e inestabilidad de la API, no como una interrupción total del registro o del servicio. \\
...
El segundo atributo prioritario es la integridad y trazabilidad, motivado por RNF-04 y por el objetivo normativo de RNF-11. Las mutaciones persistentes y sus eventos se confirman conjuntamente en PostgreSQL; los privilegios del rol operativo protegen la bitácora y la cadena SHA-256 permite comprobar su coherencia. El modelo de confianza incluye a los administradores de la base de datos y contempla el anclaje externo de la cabeza como protección adicional frente al truncamiento final. La frontera transaccional y las responsabilidades de almacenamiento se detallan en \cref{sec:postgresql}.
...
La vista de contexto distingue cuatro perfiles humanos: Owner, Litigante, Paralegal y Cliente. Estos acceden a los servicios del backend mediante una API HTTP autenticada. El sellado utiliza una TSA local, conectada a través de un puerto que permite integrar un emisor externo. El diseño de la interfaz propone navegación desde el navegador y notificaciones por correo SMTP para acompañar los flujos de gestión documental y procesal.
...
El dominio define cuentas y permisos, identidades representadas, perfiles de participantes, administración y etapas del expediente, documentos y sus versiones, evidencia criptográfica y eventos de auditoría. Los valores procesales son declaraciones del despacho; no acreditan identidad jurídica o validez de un acto. La programación de audiencias conserva valores e historia propios; sus resultados, recursos y plazos requieren modelos adicionales. La regla fundamental es que el dominio no importa ninguna otra capa del workspace; utiliza bibliotecas de bajo nivel para tipos, serialización, fechas, errores y manejo de secretos.

{\emergencystretch=2em La capa de aplicación implementa servicios de identidad, expedientes, documentos, participantes, etapas y programación de audiencias, además de los servicios criptográficos y del flujo documental fuera de línea. Sus puertos expresan las dependencias externas y fronteras transaccionales. La preparación de soportes y declaraciones se separa de su confirmación auditada; las audiencias emplean la misma separación entre preparación y confirmación, con recibos para conciliar resultados inciertos. La aplicación depende del dominio y no de los adaptadores.\par}

Los adaptadores concretan los puertos. Las entradas implementadas son la API Axum y la interfaz de línea de comandos. Las salidas incluyen PostgreSQL mediante \texttt{postgres}, sesiones Redis mediante \texttt{redis}, bibliotecas criptográficas, OpenSSL y la TSA local. Los adaptadores de archivo permanecen para el flujo fuera de línea; el proveedor remoto opcional usa \texttt{reqwest}. Los handlers WebSocket y el envío SMTP son parte del diseño previsto. Los clientes de persistencia actuales son síncronos y se aíslan del runtime HTTP mediante trabajo bloqueante acotado.
...
La \cref{tab:bounded-contexts} organiza los contextos y sus responsabilidades en los módulos propuestos para el sistema. Cada contexto distribuye sus tipos, casos de uso y adaptadores entre las capas hexagonales. Identidad, expedientes, documentos y criptografía proporcionan la base sobre la que se propone desarrollar calendario, gestión contractual y tableros de operación.

\begin{table}[H]
  \centering
  \caption{Organización modular propuesta para los contextos y sus casos de uso.}
...
Los cinco crates son: \texttt{domain} (tipos, invariantes, puertos criptográficos y errores); \texttt{application} (casos de uso y puertos de persistencia, dependiente del dominio); \texttt{infrastructure} (adaptadores concretos, dependiente de dominio y aplicación); \texttt{web} (entrega HTTP, dependiente de aplicación y dominio); y \texttt{bin} (raíz de composición, dependiente de los anteriores). Las dependencias externas efectivas constan en los manifiestos de cada crate. La \cref{fig:workspace} representa las dependencias entre capas.
...
La selección de adaptadores se realiza en la raíz de composición. El comando \texttt{serve} instancia la TSA local, los repositorios PostgreSQL y las sesiones Redis, y los conecta con los servicios de aplicación. Esta composición mantiene las decisiones de infraestructura fuera del dominio y permite sustituir un adaptador mediante otra implementación del mismo puerto.
...
El programa Rust se distribuye como ejecutable nativo, sin requerir un intérprete del lenguaje. Su operación se apoya en PostgreSQL, Redis, OpenSSL, los archivos de configuración y el material de la PKI. El tamaño del ejecutable depende del perfil de compilación y de las bibliotecas incluidas, por lo que forma parte de las mediciones de distribución.
...
El router comparte límites de admisión entre identidad, expedientes y documentos, valida cuerpos y cabeceras y responde con \texttt{Cache-Control: no-store}. La aplicación autentica el bearer opaco mediante Redis y consulta la cuenta en PostgreSQL. Para cada operación documental, el almacén transaccional revalida el rol, la pertenencia y la asociación con el expediente. La auditoría durable se confirma con la operación; el diseño del perímetro incorpora además control de orígenes y trazas de petición.

\subsubsection{Frontend: Astro con Svelte Islands}\label{sec:frontend-astro}

Para la interfaz se propone Astro como meta-framework y Svelte para los componentes interactivos, siguiendo Islands Architecture \cite{sylormiller2019}. Las páginas informativas se renderizarán como HTML, mientras el calendario, el tablero y las vistas de firma y verificación se organizarán como islas reactivas conectadas con la API. Esta separación permite desarrollar los flujos de navegador sobre los servicios de identidad, expedientes y documentos del backend.
...
La persistencia combina PostgreSQL para usuarios, expedientes, asignaciones, documentos cifrados y auditoría, con Redis para sesiones y controles efímeros de autenticación. Los clientes \texttt{postgres} y \texttt{redis} se ejecutan de forma síncrona detrás de puertos de aplicación; la capa HTTP los invoca mediante trabajo bloqueante acotado. La separación permite que las mutaciones documentales compartan una transacción con sus eventos y que el estado de sesión tenga una duración limitada. El modelo se desarrolla en \cref{sec:bd-hibrida}.

\subsubsection{Bibliotecas del workspace}\label{sec:inventario-crates}

La \cref{tab:inventario-crates} agrupa las principales bibliotecas por su responsabilidad arquitectónica. Los manifiestos \texttt{Cargo.toml} declaran las dependencias de cada crate y \texttt{Cargo.lock} fija su resolución. El workspace establece Rust 1.88 como versión mínima soportada.

\begingroup
\small
\renewcommand{\arraystretch}{1.05}
\begin{longtable}{@{}L{0.22\linewidth}L{0.22\linewidth}L{0.46\linewidth}@{}}
  \caption{Bibliotecas principales del workspace por capa arquitectónica.}
  ...
  Infraestructura datos & postgres & Cliente PostgreSQL síncrono con consultas parametrizadas \\
  Infraestructura datos & redis & Cliente Redis síncrono para sesiones y autenticación \\
  Infraestructura datos & fs2 & Bloqueo advisory de archivos compartidos entre procesos \\
  Infraestructura red & reqwest & Cliente HTTP bloqueante (adaptador remoto opcional) \\
  ...
  Errores (boundaries) & anyhow & Errores en la raíz de composición del crate \texttt{bin} \\
  Testing & mockall, proptest, tempfile & Dobles de prueba, propiedades y directorios temporales \\
\end{longtable}
\endgroup
...
{\emergencystretch=2em El sistema adopta errores tipados con \texttt{thiserror}: \texttt{Domain\allowbreak Error} expresa invariantes, \texttt{Application\allowbreak Error} representa fallos de casos de uso y puertos, y \texttt{Api\allowbreak Error} traduce resultados al contrato HTTP. Los adaptadores convierten los errores de almacenamiento en errores del puerto. La respuesta HTTP comunica el resultado sin exponer detalles internos ni secretos.\par}

La dependencia \texttt{anyhow} queda restringida al crate \texttt{bin}, que compone la aplicación y presenta errores operativos. Dominio, aplicación, infraestructura y web conservan contratos de error propios.

\subsubsection{Modelo de concurrencia asíncrona}\label{sec:concurrencia-asincrona}

Tokio atiende el transporte HTTP asíncrono, mientras los puertos de aplicación, PostgreSQL, Redis y la preparación criptográfica ejecutan trabajo síncrono mediante \texttt{spawn\_blocking}. El servidor admite por defecto ocho peticiones y dos operaciones bloqueantes simultáneas; al saturarse responde \texttt{503 server\_busy} sin una cola indefinida. Ambos presupuestos son configurables y su dimensionamiento depende de la memoria, el costo criptográfico y la capacidad del hardware de despliegue.

El permiso de trabajo bloqueante permanece ocupado hasta que termina la operación, incluso si el cliente cancela su petición HTTP. De esta forma, la admisión sigue contabilizando el trabajo que puede confirmar una mutación. Redis acota a cinco segundos la conexión TCP y la E/S posterior al handshake; la duración del handshake requiere un control adicional. El diseño operativo deberá completar los plazos de ejecución, el pooling, el transporte protegido y el apagado ordenado.

\subsubsection{Inyección de dependencias por composición}\label{sec:inyeccion-dependencias}

La inyección de dependencias se realiza explícitamente en el crate \texttt{bin}, sin contenedor de DI. En \texttt{serve\_cmd.rs} se construyen \texttt{Identity\allowbreak Service}, \texttt{Case\allowbreak Service}, \texttt{Case\allowbreak Document\allowbreak Service} y \texttt{Participant\allowbreak Service} con repositorios PostgreSQL, sesiones Redis y un \texttt{DocumentProcessor} que reúne los adaptadores criptográficos y la TSA local. Los traits permiten verificar que cada implementación satisface su puerto. El material del firmante se carga en la configuración del servidor; la sesión identifica al actor de la operación, pero no selecciona una clave privada individual por usuario.

\subsubsection{Observabilidad estructurada}\label{sec:observabilidad}

El diseño de observabilidad utiliza el ecosistema \texttt{tracing} para organizar logs operativos y propone spans por petición, identificadores de correlación y exportación a agregadores. Su desarrollo incluye una política de tratamiento de secretos y controles sobre los campos emitidos. La auditoría cumple otra responsabilidad: conserva en PostgreSQL los eventos del dominio y su encadenamiento, de modo que el registro durable de una operación no dependa de los logs del proceso.
...
El diseño del cruce entre la red pública y el perímetro contempla terminación TLS, control de orígenes y límites de transporte. Dentro de la API, los presupuestos compartidos de peticiones, trabajo bloqueante y tamaño de cuerpo acotan el consumo de recursos según \cref{sec:concurrencia-asincrona}. La protección del despliegue deberá complementar esos presupuestos con límites de conexiones y tiempos de recepción.

El cruce del perímetro al núcleo utiliza sesiones opacas revocables, validación de entradas y permisos por rol. La identidad se obtiene de la sesión y se recargan de PostgreSQL el usuario activo y su rol. Los documentos requieren además pertenencia vigente y coincidencia exacta con el expediente solicitado. Los casos de uso determinan los eventos de auditoría asociados a sus operaciones y las respuestas de rechazo que debe recibir el solicitante.

El cruce hacia datos persistentes utiliza consultas parametrizadas y cifrado de documentos mediante envelope encryption. La cuenta operativa de PostgreSQL está separada de la cuenta migradora: el servicio abre el esquema existente sin DDL y rechaza privilegios que permitan reescribir auditoría o evidencia protegida. Los propietarios y administradores forman parte de la base de confianza. Para un despliegue público se requiere incorporar TLS a las conexiones internas y proteger la configuración y los respaldos.

Los servicios externos se modelan como entidades de la zona no confiable a las que el sistema invoca explícitamente con canales seguros: un PSC externo futuro para sellado de tiempo (RFC 3161) utilizará TLS y verificación de la firma del sello recibido; la TSA local de demostración opera sin salir del perímetro del prototipo; la integración SMTP prevista requeriría protección del transporte y autenticación. En ningún caso se confía en estos servicios para operaciones de seguridad sin verificación local del resultado: un sello recibido se verifica criptográficamente contra la cadena de certificados del emisor antes de ser aceptado e incorporado al expediente.

El modelo STRIDE de Shostack \cite{shostack2014} organiza la revisión de amenazas y la asignación de controles a cada frontera. El encadenamiento de auditoría atiende la detección de alteraciones; su anclaje externo se propone para proteger también la cabeza de la historia. La revisión de disponibilidad comprende transporte y carga, mientras que la exposición de firma RSA por HTTP requiere evaluar el manejo de claves y los canales laterales. La confianza temporal se establece sobre la TSA local y deberá ampliarse al incorporar una autoridad externa.
...
La compilación comprueba el grafo de dependencias entre crates, y CI ejecuta formato, compilación, pruebas y análisis estático. Estas verificaciones sostienen las reglas de separación de capas y los contratos de sus puertos. Como ampliación del diseño de calidad se proponen comprobaciones automáticas de trazas por petición, tratamiento de secretos en logs e inventario de adaptadores. Cada nueva comprobación deberá incorporarse al pipeline junto con sus pruebas.
...
PostgreSQL relaciona usuarios, expedientes, asignaciones, participantes y documentos mediante transacciones y claves foráneas. Las raíces documentales, sus versiones y las revisiones de clasificación se conservan separadas. El directorio añade \rutarepo{case_participants} y \rutarepo{case_participant_revisions}: cada raíz tiene un expediente inmutable y cada revisión conserva valores, huella y autor. Los archivos cifrados se guardan como bytes y su evidencia como JSONB; las fichas organizativas permanecen en claro bajo autorización. La raíz del expediente conserva identidad y hechos de creación; \rutarepo{case_administration_revisions} añade título, referencia, estado y perfil penal en revisiones inmutables. \rutarepo{case_initial_stage_registrations} vincula el registro inicial de Investigación con la primera revisión del alta completa. Las asignaciones conceden acceso a cuentas, sin crear fichas de participantes. \rutarepo{case_stage_revisions} conserva adopciones y avances ordinarios, valores temporales con precisión explícita y soportes vinculados a identidad y versión documental. La revisión procesal y la administrativa son independientes; la confirmación exige perfil completo, estado activo y autorización vigente. La secuencia incorpora el registro inicial original sin duplicarlo como adopción. Las audiencias añaden raíces y revisiones de programación, referencias históricas y recibos de operación; sus resultados, recursos y plazos requieren relaciones y reglas adicionales.

Las identidades representadas añaden \rutarepo{case_subjects} y \rutarepo{case_subject_revisions}; las fichas tipificadas usan \rutarepo{case_participant_typed_revisions}, manteniendo la misma raíz e historia manual. Las decisiones de comparación, la confianza publicada y la evidencia pública de declaración tienen registros propios. Las referencias conservan versión y digest exactos; una identidad actual no sustituye los valores históricos vinculados. Los campos identificadores son datos autorizados en PostgreSQL, mientras el contacto documentado remite a soportes cifrados. La minimización de los índices no constituye un permiso adicional de reserva.

La bitácora \rutarepo{audit_events} conserva secuencia, marca temporal canónica, actor, acción, recurso y hash encadenado. Un bloqueo consultivo común ordena las transacciones mutantes de documentos, expedientes e identidad y el cálculo de la siguiente entrada. La mutación y su evento se confirman o se revierten juntos en PostgreSQL; la criptografía y las solicitudes a la TSA se preparan fuera de esa transacción. Verificación y exportación revalidan acceso y confirman su evento antes de entregar resultados. La atomicidad se limita a PostgreSQL, mientras Redis mantiene el estado efímero de autenticación por separado. Si se pierde la respuesta de una confirmación, la recuperación debe comprobar el estado persistido antes de repetir la operación.

Los privilegios de PostgreSQL permiten al rol operativo insertar y leer eventos y le impiden actualizarlos, eliminarlos o truncarlos. La evidencia sellada y la asociación del documento al expediente se preservan mediante permisos por columna, un trigger e invariantes de aplicación. La cadena SHA-256 comprueba la coherencia de los eventos conservados. Para detectar el truncamiento de su extremo se propone anclar externamente la cabeza; las tareas administrativas y los respaldos requieren, además, controles independientes.

El flujo fuera de línea conserva documentos JSON y auditoría JSONL. Su incorporación a PostgreSQL requiere detener a los escritores y proporcionar un mapa completo de documentos a expedientes existentes. La importación preserva cifrado, firmas, sellos y hashes históricos, y registra un recibo en la misma transacción. El recibo permite reconciliar el resultado y repetir la importación sin duplicarlo, conforme al procedimiento de \texttt{docs/database-operations.md}.
...
Redis mantiene el estado efímero de autenticación, con las siguientes responsabilidades:

\begin{itemize}
  \item \textbf{Sesiones opacas}: tokens aleatorios representados en almacenamiento mediante su hash, con duración fija de 24 horas desde la emisión y revocación explícita.
  \item \textbf{Desafíos MFA}: duración de cinco minutos y consumo atómico antes de comprobar un único intento de segundo factor.
  \item \textbf{Prevención de reutilización TOTP}: reclamación atómica por usuario y código aceptado dentro de la ventana temporal, con expiración de 90 segundos.
  \item \textbf{Límite de contraseña}: cinco fallos consecutivos provocan bloqueo durante la ventana de 15 minutos. El contador y su expiración se actualizan atómicamente, preservando la ventana existente.
\end{itemize}

La autorización consulta en PostgreSQL el rol y la pertenencia de cada usuario, de modo que una revocación se refleje en peticiones posteriores con la misma sesión. Para notificaciones se propone utilizar pub/sub de Redis y un adaptador WebSocket que entregue los eventos al navegador. La operación de Redis deberá contemplar recuperación y disponibilidad, porque su pérdida interrumpe las sesiones y los controles efímeros que dependen de él.

\subsection{Diagrama del Modelo de Datos}\label{sec:diagrama-datos}

La \cref{fig:modelo-datos} presenta el diseño conceptual de la persistencia híbrida: PostgreSQL concentra entidades y relaciones durables, y Redis agrupa las estructuras volátiles. Esta separación vincula las reglas de integridad referencial con la duración y revocación del estado de autenticación.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.9\linewidth]{image27.png}
  \caption{Diseño conceptual de la persistencia híbrida PostgreSQL y Redis.}
  \label{fig:modelo-datos}
\end{figure}

En PostgreSQL, las claves foráneas vinculan cada documento con su expediente y cada asignación con un usuario. La columna \texttt{audit\_events.chain} enlaza los eventos de auditoría, y los privilegios del rol operativo protegen su carácter de solo inserción. Las asignaciones administran acceso a la información; el directorio mantiene raíces y revisiones propias, sin vincular la identidad representada con una cuenta ni deducirla de nombres coincidentes. La figura describe relaciones conceptuales. Las revisiones administrativas y el registro inicial tienen las claves físicas de la \cref{sec:impl-http-administracion}; las revisiones del directorio manual se especifican en la \cref{sec:impl-http-participantes}; las identidades y fichas tipificadas, en la \cref{sec:impl-participantes-tipificados}. Las adopciones y avances ordinarios tienen el alcance de la \cref{sec:impl-http-etapas}, sin implicar un motor completo de actividad procesal.
...
\begin{figure}[H]
  \centering
  \resizebox{\linewidth}{!}{\input{figures/despliegue}}
  \caption{Diseño de despliegue con Astro+Svelte, API Rust, PostgreSQL, Redis y servicios externos.}
  \label{fig:despliegue}
\end{figure}

El backend compila a un ejecutable nativo que concentra la composición de los adaptadores. Su entorno de operación reúne PostgreSQL, Redis, OpenSSL, la configuración y el material criptográfico. El diseño del despliegue debe coordinar el arranque de estos componentes, sus permisos y la recuperación de datos.

El flujo HTTP admite la petición con los límites compartidos, extrae y valida su entrada y ejecuta el caso de uso en trabajo bloqueante. La aplicación autentica la sesión opaca y el almacén documental revalida autorización dentro de PostgreSQL. Para la interfaz de navegador se propone integrar control de orígenes, proxy TLS y correlación de trazas a lo largo de ese recorrido.

El comando \texttt{serve} selecciona \texttt{LocalOpensslTsa} para obtener los sellos de tiempo. El puerto \texttt{TimestampService} separa el protocolo de la implementación del emisor y permite conectar un proveedor remoto mediante \texttt{reqwest}. La integración con un PSC externo requiere configurar su acceso, establecer las anclas de confianza y verificar sus respuestas.

Las conexiones PostgreSQL y Redis usan clientes síncronos detrás de puertos de aplicación. La preparación del esquema corresponde a la cuenta migradora y precede al arranque con el rol operativo restringido. El diseño de operación contempla incorporar pooling, clientes asíncronos y TLS para ampliar la capacidad y protección del transporte.
...
El expediente reúne raíz estable, administración, documentos, directorio y asignaciones. Su perfil penal contiene NUC y carpeta judicial con sus autoridades, delitos ordenados e información complementaria declarada por el despacho. El alta completa confirma una primera revisión activa y un registro inicial de Investigación, vinculado exactamente a ella. Las raíces anteriores sin historia se proyectan como revisión cero sin autor ni fecha retrospectivos; el alta básica produce una revisión uno pendiente. Completar cualquiera de esas fichas no registra una etapa. El recurso procesal independiente admite adopción y avances ordinarios de Investigación a Intermedia y de Intermedia a Juicio, con soportes exactos. La programación de audiencias fija contexto y participantes históricos; los resultados de audiencia, recursos, plazos e interoperabilidad judicial conservan reglas propias pendientes.

Las revisiones administrativas conservan valores, huella y autoría. Editar exige la revisión actual esperada; cambiar únicamente estado preserva los textos vigentes dentro de la transacción. El cierre organiza el trabajo del despacho, sin declarar terminado el proceso judicial. Bloquea mutaciones documentales y del directorio, pero conserva consulta, historia, verificación y exportación autorizadas. NUC y carpeta se comparan literalmente y son únicos, por separado, entre perfiles actuales de la instancia, incluidos los cerrados. Corregirlos libera los valores anteriores al confirmar y conserva su historia; la regla no acredita unicidad nacional ni verificación institucional. La decisión se especifica en \rutarepo{docs/adr/0022-audited-penal-case-administration.md}.

Cada documento conserva identidad estable y versiones inmutables con cifrado y evidencia propios. El listado filtra la versión máxima; el historial permite elegir otra sin sustituirla. Anexar exige la versión vigente esperada para rechazar cambios concurrentes incompatibles. Consultas y acciones requieren autorización por expediente y auditoría confirmada. Los valores administrativos no se agregan al contenido cifrado ni modifican una firma o un sello anteriores.

La clasificación conserva una historia independiente de tipo, categoría y etiquetas en claro, con revisión, huella, instante y autor. El reemplazo exige la revisión esperada; la carga clasificada confirma raíz, archivo y clasificación en una transacción auditada. Los filtros toman ambas cabezas actuales. La clasificación no modifica permisos ni evidencia criptográfica, conforme a \rutarepo{docs/adr/0020-audited-document-classification.md}.

El directorio conserva fichas manuales y tipificadas, con raíces y revisiones propias, independientes de la cuenta autora. La creación fija estado activo y revisión uno; editar, archivar o reactivar añade un sucesor con revisión esperada. El cambio exclusivo de estado conserva los valores actuales y el origen de evidencia dentro de la transacción. Lecturas e historia requieren auditoría confirmada. El cierre administrativo impide mutaciones del directorio y de identidades; no acredita la condición procesal del expediente. Los datos por tipo y la declaración interna se especifican a continuación, conservando la distinción entre validación técnica y criterios jurídicos.

\input{chapters/03-participantes-tipificados}

\input{chapters/03-audiencias}

\section{Diseño Criptográfico del Prototipo}\label{sec:diseno-cripto}

Las decisiones criptográficas aplican los estándares del NIST, las RFC del IETF y las guías de OWASP del marco teórico a las restricciones jurídicas y técnicas del prototipo.
...
{\emergencystretch=3em Los documentos almacenados se cifran con AES-256-GCM conforme a NIST SP 800-38D \cite{dworkin2007}. Cada operación utiliza un nonce aleatorio de 96 bits generado con un CSPRNG. La gestión sigue un esquema de envelope encryption: una Clave de Cifrado de Clave (KEK), configurada en el servidor, envuelve una Clave de Cifrado de Datos (DEK) aleatoria propia de cada versión, conforme a NIST SP 800-38F \cite{dworkin2012}. La rotación de KEK vuelve a envolver las DEK sin recifrar los contenidos. Los Datos Adicionales Autenticados (AAD) incluyen UUID y versión, por lo que descifrar bajo un contexto distinto se rechaza. El expediente se protege mediante asociación inmutable y autorización transaccional, sin añadirlo al AAD. El historial conserva versiones inmutables y permite seleccionar su evidencia. Este mecanismo no detecta por sí solo la restauración conjunta de un estado antiguo válido de documentos y auditoría; distinguirlo del estado actual requiere un anclaje externo de continuidad.\par}
...
Las contraseñas de los usuarios se almacenan hasheadas con Argon2id (RFC 9106) \cite{biryukov2015,biryukov2021}, la variante híbrida recomendada por OWASP \cite{owasp_pwstorage2024} y NIST SP 800-63B Rev 4 \cite{nist2024} para nuevas implementaciones. Los parámetros adoptados son: memoria m = 256 MiB (262\,144 KiB), iteraciones t = 2, paralelismo p = 1, sal de 16 bytes y hash de salida de 32 bytes. En el hardware de referencia (AMD Ryzen 7 7730U, 16 hilos lógicos) cinco corridas promediaron 529.4 ms, dentro de la banda objetivo de 0.5--1.0 segundo por intento legítimo. Si el hardware de despliegue difiere, la medición debe repetirse antes de la puesta en producción.

La autenticación utiliza correo y contraseña seguida de TOTP (RFC 6238) \cite{mraihi2011} o de un código de recuperación de un solo uso. El TOTP usa HMAC-SHA1, seis dígitos, períodos de 30 segundos y secreto de 20 bytes. El inicio de sesión emite un bearer opaco revocable, mientras los certificados X.509 vinculan la firma con su clave pública. Como ampliación del acceso de los socios se propone autenticación por certificado, que requerirá su propio desafío y política de validación.
...
El alta inicial crea al primer Owner únicamente mientras no existan usuarios; las altas posteriores requieren un Owner autenticado. Al crear la cuenta, la API devuelve una sola vez el secreto TOTP, su URI \texttt{otpauth} y ocho códigos de recuperación. El secreto se conserva cifrado con AES-256-GCM y vinculado al UUID del usuario, y los códigos se almacenan mediante Argon2id. Para la interfaz de enrolamiento se propone presentar la URI como QR y confirmar la configuración con un código generado por el dispositivo; el diseño de entrega deberá resolver también la pérdida de la respuesta inicial.

Tras validar la contraseña se emite un desafío de cinco minutos. Cada intento TOTP o de recuperación consume ese desafío atómicamente; si falla, se necesita otro inicio con contraseña. El primitivo TOTP acepta la tolerancia de un período a cada lado del instante actual, y una reclamación atómica en Redis impide reutilizar el código aceptado. Los fallos consecutivos de contraseña se contabilizan por separado: al alcanzar cinco se bloquea el acceso durante el tiempo restante de la ventana de 15 minutos.

La autorización usa el rol y la cuenta activos de PostgreSQL. Owner ve todos los expedientes; Litigante y Paralegal consultan la administración de los asignados, mientras Cliente conserva únicamente su proyección básica. Owner y Litigante administran perfiles y estado; las asignaciones siguen reservadas a Owner. Para documentos, Litigante y Paralegal necesitan asignación vigente además del permiso de la \cref{tab:permisos-implementados}; Cliente permanece denegado incluso asignado. Se comprueba la asociación exacta documento/expediente también para Owner. Una revocación confirmada antes de la transacción documental impide que esta se confirme; si la operación obtuvo primero el bloqueo, confirma antes y la revocación espera. Una respuesta ya confirmada puede terminar de transmitirse después de esa revocación.

\begin{table}[H]
  \centering
  \caption{Permisos de administración, documentos y directorio.}
  \label{tab:permisos-implementados}
  \begin{tabularx}{\linewidth}{@{}X C{0.12\linewidth} C{0.14\linewidth} C{0.14\linewidth} C{0.12\linewidth}@{}}
    \toprule
    \textbf{Operación} & \textbf{Owner} & \textbf{Litigante} & \textbf{Paralegal} & \textbf{Cliente} \\
    \midrule
    Consultar administración e historial & Sí & Sí & Sí & No \\
    Registrar o administrar expediente & Sí & Sí & No & No \\
    Consultar metadatos documentales & Sí & Sí & Sí & No \\
    Cargar o añadir versión & Sí & Sí & Sí & No \\
    Clasificar documento & Sí & Sí & Sí & No \\
    Sellar documento & Sí & Sí & No & No \\
    Verificar documento & Sí & Sí & Sí & No \\
    Exportar evidencia & Sí & Sí & Sí & No \\
    Consultar etapa e historial & Sí & Sí & Sí & No \\
    Adoptar o avanzar etapa & Sí & Sí & No & No \\
    Consultar directorio e historial & Sí & Sí & Sí & No \\
    Crear, editar o archivar ficha & Sí & Sí & No & No \\
    Verificar auditoría completa & Sí & No & No & No \\
    \bottomrule
  \end{tabularx}
\end{table}

Los permisos de la tabla se combinan con pertenencia vigente, recurso exacto y estado administrativo. Un expediente cerrado conserva las lecturas y operaciones de evidencia, pero rechaza nuevas cargas, versiones, clasificación, sellado, edición de perfil, mutaciones del directorio y registros de etapa. Reactivar no modifica etapas ni reactiva participantes archivados.

\subsection{Postura Legal del Prototipo}\label{sec:postura-legal}

La firma generada por el prototipo constituye una firma electrónica simple conforme al artículo 89 del Código de Comercio \cite{ccom2018}, dado que los certificados no provienen de un PSC autorizado (artículo 97, requisito 5). Su argumentación probatoria se apoya en: (1) la admisibilidad de la firma simple como prueba, ya que el artículo 89 no condiciona la admisibilidad a la intervención de un PSC; y (2) la cláusula de equivalencia funcional del artículo 100, que permite argumentar la fiabilidad del método con base en estándares reconocidos internacionalmente (RSA-3072 conforme a NIST SP 800-57 \cite{barker2020} y SHA-256 conforme a FIPS 180-4 \cite{nist2015fips1804}). Un sello emitido por un PSC autorizado añadiría fecha cierta e independencia de tercero conforme al marco de la NOM-151; el token de la TSA interna demuestra el mecanismo RFC 3161, pero no añade esa garantía externa. La distinción práctica respecto de una firma avanzada radica en la carga de la prueba: quien invoca la firma simple debe acreditar la fiabilidad del método, mientras que la firma avanzada goza de presunción legal. La sustitución de la CA interna requiere evaluar identidad, custodia de claves, políticas y verificación del emisor; no se infiere una categoría jurídica distinta únicamente de cambiar el certificado.

Los puertos criptográficos separan el formato de evidencia de la implementación del emisor. Esta organización permite integrar certificados y autoridades externas compatibles mediante adaptadores específicos. La adopción de infraestructura regulada requiere evaluar la identidad y custodia de las claves, la confianza en el proveedor y las condiciones del flujo completo, junto con la verificación técnica de las firmas y los sellos.
