...
\paragraph{Alcance del diseño y corte de implementación: 12 de septiembre de 2026.}
Este capítulo conserva las historias de usuario, los casos de uso y el cronograma como especificación del producto previsto; su presencia no acredita que cada función esté implementada. El corte revisado incorpora un backend HTTP autenticado, expedientes con título, referencia y asignaciones de usuarios, operaciones documentales vinculadas al expediente y auditoría transaccional en PostgreSQL. La interfaz de producto, los participantes procesales, las audiencias, los plazos y las transiciones de etapa permanecen pendientes. Tampoco existen todavía listado, búsqueda ni historial de versiones documentales; cada carga crea la versión 1.

La autenticación vigente utiliza correo y contraseña con segundo factor, y sesiones opacas revocables en Redis. La autorización combina el rol actual con la asignación al expediente; Cliente puede leer metadatos de expedientes asignados, pero conserva denegadas todas las operaciones documentales. La persistencia HTTP de documentos cifrados y auditoría es PostgreSQL; los archivos JSON y JSONL se mantienen para el flujo fuera de línea y como origen de una importación controlada. La transacción de PostgreSQL no abarca las sesiones ni los desafíos de Redis.

Este corte se contrasta con el contrato versionado \texttt{docs/http-api.md}, las migraciones SQL y las decisiones de \texttt{docs/adr/}: sesiones revocables (0012), pertenencia a expedientes (0014), límites e invariantes del backend (0015) y documentos con auditoría transaccional (0016). Los apartados siguientes distinguen esos mecanismos de las vistas y servicios todavía propuestos. La TSA de la demostración es local; esta implementación no acredita cumplimiento de la NOM-151 ni disponibilidad de un PSC externo.
...
\begin{itemize}
  ...
  \item \textbf{Iteración 9 (Semana 9) - Persistencia, Control de Acceso y Gestión Procesal:} Implementación de la persistencia híbrida PostgreSQL/Redis y del control de acceso basado en roles (RBAC) y asignación al expediente. La alternativa inicial JWT se sustituyó por sesiones opacas revocables. Desarrollo de la gestión procesal y documental —módulos CRUD para expedientes, casos, contratos y documentos— integrando los servicios criptográficos ya probados. Desarrollo de una interfaz web mínima.
...
La numeración semanal expresa el orden del plan original, no semanas ya concluidas. Al corte indicado, la persistencia, las asignaciones y el flujo documental transaccional representan un avance parcial de la iteración 9; no se declara terminado el CRUD procesal ni la interfaz mínima. Las pruebas de usabilidad y el cierre de la iteración 10 requieren completar y evaluar esas funciones.
...
El segundo atributo prioritario es la integridad y trazabilidad, motivado por RNF-04 y por el objetivo normativo de RNF-11. En la implementación revisada, las mutaciones persistentes y sus eventos se confirman conjuntamente en PostgreSQL; el rol operativo carece de permisos para modificar o eliminar auditoría, y la cadena SHA-256 permite detectar inconsistencias. Estas medidas no hacen imposible la intervención de un administrador de base de datos ni prueban por sí mismas la ausencia de truncamiento final: el anclaje externo de la cabeza sigue pendiente. El alcance técnico se detalla en \cref{sec:postgresql}, sin equipararlo con cumplimiento normativo.
...
La vista de contexto prevista contempla actores del despacho y del cliente, navegadores, un proveedor de sellado de tiempo externo y correo SMTP para notificaciones. Al corte de implementación, la API distingue Owner, Litigante, Paralegal y Cliente, y utiliza una TSA local. El correo, las notificaciones y la interfaz de navegador descritos en esta vista son componentes futuros; no se presenta una integración productiva con Cincel.
...
El dominio define identidades y permisos, metadatos de expediente, documentos y sus versiones, evidencia criptográfica y eventos de auditoría. El diseño jurídico añade participantes, audiencias y etapas procesales como conceptos previstos, todavía sin implementación. La regla fundamental es que el dominio no importa ninguna otra capa del workspace; utiliza bibliotecas de bajo nivel para tipos, serialización, fechas, errores y manejo de secretos.

{\emergencystretch=2em La capa de aplicación implementa \texttt{Identity\allowbreak Service}, \texttt{Case\allowbreak Service} y \texttt{Case\allowbreak Document\allowbreak Service}, además de los servicios criptográficos y del flujo documental fuera de línea. Sus puertos expresan las dependencias externas, incluida la frontera transaccional \texttt{Case\allowbreak Document\allowbreak Store}. La programación de audiencias y la transición de etapas son casos de uso previstos. La aplicación depende del dominio y no de los adaptadores.\par}
...
La \cref{tab:bounded-contexts} conserva el reparto conceptual de responsabilidades y los nombres de módulos propuestos; no es un inventario de módulos compilados ni una declaración de cumplimiento de todos sus casos de uso. El código actual agrupa identidad, expedientes, documentos y criptografía dentro de cada capa hexagonal; un contexto puede atravesar varios crates. Calendario, contratos como módulo independiente y tableros siguen pendientes.

\begin{table}[H]
  \centering
  \caption{Asignación prevista de contextos, casos de uso y módulos; no expresa estado de implementación.}
...
La selección de adaptadores se realiza explícitamente en la raíz de composición. El comando \texttt{serve} instancia la TSA local y los repositorios PostgreSQL; no se implementaron las features \texttt{cincel-tsa} y \texttt{seguridata-tsa} contempladas inicialmente. Los puertos permiten una sustitución posterior sin modificar el dominio, pero esa posibilidad no prueba la operación de un proveedor externo.
...
El programa Rust se distribuye como ejecutable nativo, sin requerir un intérprete del lenguaje. Esto no elimina las dependencias de operación: la demostración necesita PostgreSQL, Redis, OpenSSL, configuración y material de la PKI. El tamaño del ejecutable depende del perfil y de las bibliotecas incluidas; no se utiliza una estimación inicial como medición de esta entrega.
...
El router actual comparte límites de admisión entre identidad, expedientes y documentos, valida cuerpos y cabeceras y responde con \texttt{Cache-Control: no-store}. El bearer opaco se autentica en la aplicación mediante Redis y la cuenta actual en PostgreSQL; la autorización documental se revalida en la transacción. El pipeline más amplio previsto con CORS, trazas de petición y controles de despliegue no se presenta como una implementación completa. La auditoría durable se produce en los casos de uso y adaptadores transaccionales, no como un log genérico del middleware.

\subsubsection{Frontend: Astro con Svelte Islands}\label{sec:frontend-astro}

El frontend previsto combina Astro como meta-framework y Svelte para componentes interactivos, siguiendo Islands Architecture \cite{sylormiller2019}. La base tecnológica existe, pero las páginas actuales son placeholders sin integración con la API. El calendario, el tablero y las vistas de firma y verificación aquí descritos representan el diseño de interfaz pendiente, no pantallas disponibles.
...
La implementación combina PostgreSQL para usuarios, expedientes, asignaciones, documentos cifrados y auditoría, con Redis para sesiones y controles efímeros de autenticación. Los clientes son \texttt{postgres} y \texttt{redis}, ambos síncronos. La elección inicial de \texttt{sqlx}/\texttt{fred} y las notificaciones pub/sub permanecen como alternativas de evolución; no hay verificación de consultas SQL en compilación ni notificaciones en tiempo real implementadas. La frontera de persistencia se describe en \cref{sec:bd-hibrida}.

\subsubsection{Inventario completo de crates}\label{sec:inventario-crates}

La \cref{tab:inventario-crates} distingue las bibliotecas utilizadas de las alternativas aún previstas. Los manifiestos \texttt{Cargo.toml} declaran dependencias y \texttt{Cargo.lock} fija su resolución. El MSRV vigente es Rust 1.88; el listado no implica que una biblioteca declarada en el workspace sea consumida por todos sus miembros.

\begin{longtable}{@{}L{0.22\linewidth}L{0.22\linewidth}L{0.46\linewidth}@{}}
  \caption{Bibliotecas utilizadas y alternativas previstas, agrupadas por capa arquitectónica.}
  ...
  Aplicación (previsto) & async-trait & No consumido por los puertos síncronos actuales \\
  ...
  Infraestructura red (previsto) & lettre & Integración SMTP pendiente \\
  ...
  Validación (previsto) & validator & La validación actual usa tipos y reglas propias \\
  ...
  Testing & mockall, proptest, tempfile & Dobles, propiedades y fixtures; rstest y wiremock no se utilizan \\
\end{longtable}
...
Tokio atiende el transporte HTTP asíncrono, mientras los puertos de aplicación, PostgreSQL, Redis y la preparación criptográfica ejecutan trabajo síncrono mediante \texttt{spawn\_blocking}. El servidor admite por defecto ocho peticiones y dos operaciones bloqueantes simultáneas; al saturarse responde \texttt{503 server\_busy} sin una cola indefinida. Son presupuestos configurables que deben medirse en el hardware de despliegue, no una garantía de concurrencia ilimitada.

La cancelación de una petición HTTP no detiene automáticamente el trabajo bloqueante ni revierte una mutación. Su permiso permanece ocupado hasta que termina la operación. No existe un timeout global que garantice cancelar todo el flujo: Redis acota conexión TCP y E/S posterior al handshake a cinco segundos, pero el handshake del cliente actual queda fuera de ese límite. Pooling, transporte protegido, plazos completos y apagado ordenado permanecen como trabajo de despliegue.

\subsubsection{Inyección de dependencias por composición}\label{sec:inyeccion-dependencias}

La inyección de dependencias se realiza explícitamente en el crate \texttt{bin}, sin contenedor de DI. En \texttt{serve\_cmd.rs} se construyen \texttt{Identity\allowbreak Service}, \texttt{Case\allowbreak Service} y \texttt{Case\allowbreak Document\allowbreak Service} con repositorios PostgreSQL, sesiones Redis y un \texttt{DocumentProcessor} que reúne los adaptadores criptográficos y la TSA local. Los traits permiten verificar que cada implementación satisface su puerto. El material del firmante se carga en la configuración del servidor; la sesión identifica al actor de la operación, pero no selecciona una clave privada individual por usuario.

\subsubsection{Observabilidad estructurada}\label{sec:observabilidad}

La observabilidad propuesta utiliza el ecosistema \texttt{tracing} y contempla spans, correlación de peticiones y exportación a agregadores. La disponibilidad de estas bibliotecas no acredita un pipeline completo por petición, un filtro de secretos mediante expresiones regulares ni reconfiguración en caliente. La trazabilidad durable implementada es la cadena PostgreSQL de eventos del dominio; debe distinguirse de los logs operativos. El diseño de observabilidad productiva sigue requiriendo validación específica.
...
Para el cruce de la red pública al perímetro se prevén terminación TLS, control de orígenes y límites de transporte. Estos controles de despliegue no están certificados por la demostración local. El backend sí aplica los límites compartidos de peticiones, trabajo bloqueante y tamaño de cuerpo descritos en \cref{sec:concurrencia-asincrona}; no se afirma que implemente un WAF ni una ventana deslizante general de protección de la API.

El cruce del perímetro al núcleo utiliza sesiones opacas revocables, validación de entradas y permisos por rol. El usuario activo y su rol se recargan desde PostgreSQL; los documentos requieren además pertenencia vigente y coincidencia exacta con el expediente solicitado. La identidad no procede de un JWT ni de una cabecera libre con el actor. Una denegación no implica que toda petición rechazada genere un evento durable: el registro corresponde a las operaciones instrumentadas por la aplicación.

El cruce hacia datos persistentes utiliza consultas parametrizadas y cifrado de documentos mediante envelope encryption. La cuenta operativa de PostgreSQL está separada de la cuenta migradora: el servicio abre el esquema existente sin DDL y rechaza privilegios que permitan reescribir auditoría o evidencia protegida. Las conexiones actuales de demostración no acreditan TLS interno; incorporarlo forma parte del despliegue pendiente. Los propietarios y administradores de la base siguen dentro de la base de confianza.
...
El modelo STRIDE de Shostack \cite{shostack2014} organiza la revisión prevista de amenazas y los controles asignados a cada frontera. Una matriz de diseño no demuestra por sí sola cobertura efectiva. Persisten limitaciones relevantes: anclaje externo de auditoría, transporte, carga y revisión de la firma RSA expuesta por HTTP. La TSA local tampoco aporta la independencia del tercero externo considerado por el diseño.
...
La compilación comprueba el grafo de dependencias entre crates, y CI ejecuta formato, compilación, pruebas y análisis estático. Las comprobaciones propuestas de spans en cada handler, saneamiento de logs, inventario automático de implementaciones de puertos y tamaño máximo del binario no deben darse por ejecutadas: no corresponden a verificaciones implementadas en el pipeline revisado. Los resultados reproducidos y su alcance se presentan en el capítulo de pruebas.
...
PostgreSQL proporciona las transacciones y claves foráneas necesarias para relacionar usuarios, expedientes, asignaciones y documentos. El esquema implementado consta de \rutarepo{users}, \rutarepo{cases}, \rutarepo{case_memberships}, \rutarepo{documents}, \rutarepo{audit_events} y \rutarepo{migration_receipts}. El contenido documental cifrado se guarda como bytes y la evidencia capturada como JSONB. Los metadatos variables de proceso, el uso de \texttt{pgcrypto} y las direcciones IP de la bitácora pertenecen al diseño previsto y no son columnas o mecanismos actuales.

La bitácora \rutarepo{audit_events} conserva secuencia, marca temporal canónica, actor, acción, recurso y hash encadenado. Un bloqueo consultivo común ordena las transacciones mutantes de documentos, expedientes e identidad y el cálculo de la siguiente entrada. La mutación y su evento se confirman o se revierten juntos en PostgreSQL; la criptografía y las solicitudes a la TSA se preparan fuera de esa transacción. Verificación y exportación revalidan acceso y confirman su evento antes de entregar resultados. Esta atomicidad no se extiende a Redis ni permite interpretar una pérdida de respuesta de red como prueba de rollback.

El rol operativo puede insertar y leer eventos, pero no actualizarlos, eliminarlos ni truncarlos. La protección utiliza privilegios de PostgreSQL; no la regla \texttt{RULE} del diseño inicial. La evidencia sellada y la asociación del documento al expediente se preservan mediante permisos por columna, un trigger e invariantes de aplicación. La cadena SHA-256 permite comprobar la coherencia de los eventos conservados, pero sin un anclaje externo no demuestra la ausencia de truncamiento al final. Las tareas administrativas y los respaldos siguen requiriendo controles independientes.

Los documentos JSON y la auditoría JSONL del flujo fuera de línea pueden importarse con los escritores detenidos y un mapa completo a expedientes existentes. La importación preserva cifrado, firmas, sellos y hashes históricos, y registra un recibo en la misma transacción. No se reemite evidencia para migrarla. El procedimiento se documenta en \texttt{docs/database-operations.md}.
...
En el backend actual, Redis mantiene estado efímero de autenticación, con las siguientes responsabilidades:

\begin{itemize}
  \item \textbf{Sesiones opacas}: tokens aleatorios cuya representación persistida no contiene el token en claro, con duración de 24 horas y revocación explícita. La duración es fija desde la emisión, no una política de inactividad deslizante.
...
No se cachean permisos ni pertenencias en Redis: una petición consulta el estado actual en PostgreSQL. Caché, notificaciones pub/sub, WebSocket y recuperación de contraseña por correo son capacidades previstas. La pérdida de Redis puede interrumpir sesiones y controles efímeros; su persistencia y operación deben evaluarse antes del despliegue.

\subsection{Diagrama del Modelo de Datos}\label{sec:diagrama-datos}

La \cref{fig:modelo-datos} conserva el modelo conceptual propuesto; sus tablas y TTL no constituyen un volcado del esquema implementado. Para el corte actual prevalecen las tres migraciones SQL y las responsabilidades de PostgreSQL y Redis descritas en las subsecciones anteriores.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.9\linewidth]{image27.png}
  \caption{Modelo híbrido previsto; el esquema y los TTL implementados se precisan en el texto.}
  \label{fig:modelo-datos}
\end{figure}

Los nombres \texttt{audit\_log}, \texttt{sha256\_chain}, la regla \texttt{RULE} y \texttt{cases.metadata} de la figura corresponden al diseño inicial. El esquema vigente usa \texttt{audit\_events.chain}, privilegios de solo inserción para el rol operativo y expedientes con título y referencia; las asignaciones de acceso no equivalen a participantes procesales.
...
\begin{figure}[H]
  ...
  \caption{Despliegue previsto: Astro+Svelte, API Rust, PostgreSQL, Redis y servicios externos; la demostración actual usa API y TSA locales.}
  \label{fig:despliegue}
\end{figure}

El backend compila a un ejecutable nativo que concentra la composición de los adaptadores. Para operarlo también se requieren PostgreSQL, Redis, OpenSSL y el material criptográfico configurado. La distribución del binario no constituye por sí sola un despliegue completo ni evidencia de disponibilidad productiva.

El flujo implementado admite la petición con los límites compartidos, extrae y valida su entrada y ejecuta el caso de uso en trabajo bloqueante. La aplicación autentica la sesión opaca y el almacén documental revalida autorización dentro de PostgreSQL. CORS, proxy TLS y observabilidad ampliada del diagrama son componentes de despliegue previstos.

El comando \texttt{serve} selecciona explícitamente \texttt{LocalOpensslTsa}. El adaptador remoto mediante \texttt{reqwest} permanece disponible como integración opcional, pero una campaña con PSC externo no forma parte de la demostración. El puerto \texttt{TimestampService} permite sustituir la implementación del emisor sin modificar el dominio.

Las conexiones PostgreSQL y Redis usan clientes síncronos detrás de puertos de aplicación. El arranque operativo requiere migraciones previas y un rol restringido; no instala ni modifica tablas. El pooling, clientes asíncronos y TLS son tareas posteriores, no propiedades inferidas de la presencia de Tokio en el transporte HTTP.
...
En el modelo jurídico previsto, el caso evoluciona entre etapas procesales \cite{cnpp2021}, con documentos habilitantes e historial de transiciones. El corte implementado solo representa título, referencia, creador y asignaciones de usuarios. La vinculación documental ya existe, pero no impone todavía estados procesales ni valida escritos de acusación o autos de apertura. Participantes, audiencias y plazos deben modelarse como funciones distintas de las asignaciones que hoy autorizan el acceso.
...
Todos los roles del backend actual utilizan contraseña seguida de TOTP (RFC 6238) \cite{mraihi2011} o de un código de recuperación de un solo uso. El TOTP usa HMAC-SHA1, seis dígitos, períodos de 30 segundos y secreto de 20 bytes. La autenticación de socios mediante certificado X.509 permanece como requisito futuro; los certificados de firma no son credenciales de inicio de sesión en esta implementación. La selección de mecanismos no se presenta como una evaluación de conformidad AAL2 o AAL3.
...
El alta inicial crea al primer Owner únicamente mientras no existan usuarios; las altas posteriores requieren un Owner autenticado. Al crear la cuenta, la API devuelve una sola vez el secreto TOTP, su URI \texttt{otpauth} y ocho códigos de recuperación; no hay todavía interfaz QR ni confirmación de enrolamiento separada. El secreto se conserva cifrado con AES-256-GCM y vinculado al UUID del usuario, y los códigos se almacenan mediante Argon2id. La pérdida de esa respuesta requiere resolver la entrega de enrolamiento: no se presume un mecanismo de reenvío implementado.

Tras validar la contraseña se emite un desafío de cinco minutos. Cada intento TOTP o de recuperación consume ese desafío atómicamente; si falla, se necesita otro inicio con contraseña. El primitivo TOTP acepta la tolerancia de un período a cada lado del instante actual; el flujo HTTP añade una reclamación Redis que impide reutilizar el código aceptado. El límite de cinco fallos y 15 minutos corresponde a contraseñas, no a reintentos ilimitados del mismo desafío MFA.
...
\begin{table}[H]
  \centering
  \caption{Permisos documentales implementados al 12 de septiembre de 2026.}
...
Los puertos criptográficos permiten estudiar posteriormente certificados y autoridades externas compatibles, manteniendo separados el formato de evidencia y la implementación del emisor. Esa posibilidad técnica no acredita por sí misma la confianza del proveedor ni el cumplimiento del flujo completo. La demostración evaluada utiliza certificados y TSA locales; cualquier adopción de infraestructura regulada deberá verificarse expresamente, sin presentar el cambio de configuración como validación jurídica automática.