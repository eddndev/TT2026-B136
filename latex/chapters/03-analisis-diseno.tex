...
\textbf{Fase II: Construcción, Pruebas y Refactorización}. Una vez validado el diseño, el desarrollo entra en un ciclo estricto de codificación iterativa mediante XP, utilizando programación en pares, Desarrollo Guiado por Pruebas (TDD) y entregas continuas.

\begin{itemize}
  \item \textbf{Iteración 6 (Semana 6) - Base del Sistema y Control de Acceso:} Configuración del entorno de producción e implementación del esquema de base de datos híbrida. Desarrollo de la API de negocio básica y el sistema de control de acceso basado en roles (RBAC).

  \item \textbf{Iteración 7 (Semana 7) - Gestión Procesal y Documental:} Desarrollo de la interfaz de usuario (UI). Implementación de los módulos CRUD para expedientes, casos, contratos y documentos. Habilitación de la carga y visualización de documentos legales.

  \item \textbf{Iteración 8 (Semana 8) - Implementación de Confidencialidad e Integridad:} Integración de cifrado autenticado en el almacenamiento de expedientes y documentos, proporcionando confidencialidad (protección contra accesos no autorizados) e integridad (detección de modificaciones no autorizadas). Implementación de algoritmos de huella digital (Hashing) para verificación independiente de integridad en flujos que no requieran confidencialidad.

  \item \textbf{Iteración 9 (Semana 9) - Implementación del No Repudio (Contratos):} Integración de la firma digital (X.509) en el flujo de aceptación de contratos. Vinculación con los servicios de Autoridad de Sellado de Tiempo para el cumplimiento de la NOM-151.
...
\begin{table}[H]
  ...
  \begin{tabularx}{\linewidth}{@{}>{\bfseries}l X@{}}
    ...
    Postcondiciones & El tenant queda creado con aislamiento lógico, el plan activo y el socio fundador autenticado con rol Owner. Las credenciales se almacenan mediante funciones de hashing resistentes. \\
...
\begin{table}[H]
  ...
  \begin{tabularx}{\linewidth}{@{}>{\bfseries}l X@{}}
    ...
    Flujo principal (alta) & 1. El Owner accede al módulo de gestión de equipo. 2. Ingresa el correo electrónico del nuevo miembro y selecciona el perfil a asignar (Litigante o Paralegal). 3. El sistema verifica que el tenant no ha alcanzado el límite de usuarios del plan contratado. 4. El sistema genera un enlace de invitación con vigencia limitada y lo envía por correo. 5. El colaborador invitado accede al enlace, completa su registro con sus datos personales y establece su contraseña. 6. El sistema valida los datos, crea la cuenta asociada al tenant con el perfil indicado y registra la operación en la bitácora. 7. El nuevo miembro accede al sistema con las funciones habilitadas según su perfil. \\
    ...
    Flujos alternos & 2a. (Alta) El correo ya pertenece a un miembro activo del despacho → el sistema rechaza la invitación e informa el motivo. 3a. (Alta) El tenant ha alcanzado el límite de usuarios → el sistema bloquea la invitación e indica la necesidad de actualizar el plan. 5a. (Alta) El enlace de invitación ha expirado → el sistema muestra un mensaje informativo; el Owner debe reenviar la invitación. 2b. (Baja) El Owner intenta darse de baja a sí mismo → el sistema rechaza la operación, ya que el tenant requiere al menos un Owner activo. \\
    Postcondiciones & (Alta) El colaborador queda registrado en el tenant con el perfil asignado. (Baja) El acceso del colaborador queda revocado de inmediato. En ambos casos, la operación se registra en la bitácora de actividad. \\
...
\begin{table}[H]
  ...
  \begin{tabularx}{\linewidth}{@{}>{\bfseries}l X@{}}
    ...
    Flujos alternos & 3a. El perfil no tiene permiso sobre el módulo solicitado → el sistema deniega el acceso con un mensaje explicativo y registra el intento en la bitácora. 3b. El usuario intenta acceder a un expediente de un tenant diferente al suyo → el sistema devuelve un error de autorización sin revelar la existencia del recurso. \\
...
\begin{table}[H]
  ...
  \begin{tabularx}{\linewidth}{@{}>{\bfseries}l X@{}}
    ...
    Precondiciones & El Litigante tiene sesión activa y pertenece al tenant. \\
    Flujo principal & 1. El Litigante accede al módulo de gestión procesal y selecciona la opción de nuevo expediente. 2. Ingresa los identificadores oficiales: Número Único de Caso (NUC) y Carpeta Judicial. 3. Registra el tipo de delito, los datos generales del asunto y los metadatos asociados. 4. El sistema valida que los campos obligatorios estén completos y que los identificadores no estén duplicados dentro del tenant. 5. El sistema persiste el expediente con estado inicial “Investigación”, genera la marca de tiempo de creación y registra la operación en la bitácora. \\
    Flujos alternos & 4a. Faltan campos obligatorios (NUC o Carpeta Judicial) → el sistema rechaza el registro e indica los campos faltantes. 4b. El NUC ya existe en el tenant → el sistema informa la duplicidad. \\
    Postcondiciones & El expediente queda registrado con estado “Investigación”, disponible para los usuarios autorizados del tenant. \\
...
\begin{table}[H]
  ...
  \begin{tabularx}{\linewidth}{@{}>{\bfseries}l X@{}}
    ...
    Postcondiciones & El calendario judicial del tenant queda actualizado. Todos los cómputos de plazos futuros utilizan la configuración vigente. \\
    Derivado de & HU-22 \\
...
\begin{table}[H]
  ...
  \begin{tabularx}{\linewidth}{@{}>{\bfseries}l X@{}}
    ...
    RF relacionados & RF-12, RF-13, RF-14, RF-15 \\
    RNF relacionados & RNF-03, RNF-04 \\
    \bottomrule
  \end{tabularx}
\end{table}
...
\begin{table}[H]
  ...
  \begin{tabularx}{\linewidth}{@{}>{\bfseries}l X@{}}
    ...
    Actores secundarios & PSC autorizado (Cincel, como Autoridad de Sellado de Tiempo) \\
    ...
    Flujo principal & 1. El Litigante accede al contrato y selecciona la opción de firma digital. 2. El sistema presenta el contenido del contrato y solicita confirmación explícita de la acción de firma. 3. El Litigante confirma la firma. 4. El sistema calcula la huella digital del contrato. 5. El sistema firma la huella digital con la clave privada del certificado X.509 del firmante, vinculando criptográficamente su identidad con el contenido del documento. 6. El sistema envía la huella digital al PSC autorizado mediante el protocolo de sellado de tiempo. 7. El PSC retorna el token de sello de tiempo que acredita la fecha y hora exacta de la operación. 8. El sistema almacena el contrato firmado junto con la firma digital, los metadatos del certificado y el token de sello de tiempo. 9. La operación se registra en la bitácora de actividad y en la cadena de custodia del expediente. \\
    Flujos alternos & 2a. El certificado del firmante ha expirado o está revocado → el sistema informa la situación e impide la firma. 6a. El PSC no responde dentro del tiempo límite → el sistema encola la solicitud para reintento automático y notifica al Litigante del retraso en la constancia. 7a. El token recibido no supera la validación de formato → el sistema rechaza el sello, registra el error y reintenta con el PSC. \\
    Postcondiciones & El contrato queda firmado con evidencia criptográfica verificable de la identidad del firmante y del momento exacto de la firma. La constancia tiene validez conforme a la NOM-151-SCFI-2016. \\
...
\begin{table}[H]
  ...
  \begin{tabularx}{\linewidth}{@{}L{3.2cm}X@{}}
    ...
    \textbf{Actores involucrados} & Todos los roles (Owner, Litigante, Paralegal); sistema de gestión de sesiones. \\
...
\begin{table}[H]
  ...
  \begin{tabularx}{\linewidth}{@{}L{3.2cm}X@{}}
    ...
    \textbf{Descripción} & El sistema debe asignar permisos de forma granular a través de roles predefinidos (Owner, Litigante, Paralegal) para restringir el acceso a módulos y operaciones sensibles. \\
...
\begin{table}[H]
  ...
  \begin{tabularx}{\linewidth}{@{}L{3.2cm}X@{}}
    ...
    \textbf{Salida} & Invitación enviada; usuario creado y asociado al tenant; acceso habilitado según el rol asignado. \\
...
\begin{table}[H]
  ...
  \begin{tabularx}{\linewidth}{@{}L{3.2cm}X@{}}
    ...
    \textbf{Entrada} & Documento o expediente a cifrar; clave maestra del tenant; IV generado aleatoriamente por operación. \\
...
\begin{table}[H]
  ...
  \begin{tabularx}{\linewidth}{@{}L{3.2cm}X@{}}
    ...
    \textbf{Descripción} & El sistema debe obtener una constancia de conservación mediante un PSC autorizado para cada contrato firmado, conforme al protocolo RFC 3161 (véase §2.3.3.4), acreditando la existencia del documento en un momento determinado. \\
    \textbf{Entrada} & Hash SHA-256 del contrato firmado; solicitud al PSC autorizado (Cincel) mediante API RFC 3161. \\
    \textbf{Salida} & Token de sello de tiempo (TimeStampToken) en formato ASN.1/.tsr almacenado y vinculado al contrato; constancia de conservación NOM-151 emitida. \\
    \textbf{Actores involucrados} & Sistema backend (cliente TSA); PSC autorizado (Cincel como TSA); módulo de firma digital. \\
    \textbf{Criterio de aceptación} & Dado un contrato firmado, cuando se solicita el sello al PSC, entonces el sistema recibe y almacena un token RFC 3161 válido en menos de 10 segundos; el token debe ser verificable de forma independiente con OpenSSL sin requerir acceso al sistema. \\
    \bottomrule
  \end{tabularx}
\end{table}
...
\begin{table}[H]
  ...
  \begin{tabularx}{\linewidth}{@{}L{3.2cm}X@{}}
    ...
    \textbf{Entrada} & Datos de casos activos, plazos próximos y asignación de carga del tenant activo. \\
...
\begin{table}[H]
  \centering
  \caption{RNF-01: Aislamiento lógico multi-tenant}
  \label{tab:rnf-01}
  \begin{tabularx}{\linewidth}{@{}L{3.2cm}X@{}}
    \toprule
    \textbf{ID} & RNF-01 \\
    \midrule
    \textbf{Nombre} & Aislamiento lógico multi-tenant \\
    \textbf{Descripción} & El sistema debe garantizar la segregación estricta de datos entre despachos, impidiendo que cualquier consulta, operación o error de lógica exponga información de un tenant a otro. \\
    \textbf{Entrada} & Consulta o operación de cualquier usuario autenticado sobre cualquier recurso del sistema. \\
    \textbf{Salida} & Respuesta que contiene únicamente datos del tenant al que pertenece el usuario autenticado. \\
    \textbf{Actores involucrados} & Todos los usuarios; capa de middleware de aislamiento; base de datos relacional. \\
    \textbf{Criterio de aceptación} & Dado un usuario de tenant A, cuando realiza cualquier consulta (incluso manipulando parámetros), entonces el sistema nunca devuelve datos de tenant B; verificable mediante pruebas automatizadas que simulen inyección de \texttt{tenant\_id} ajeno. \\
    \bottomrule
  \end{tabularx}
\end{table}
...
\begin{table}[H]
  \centering
  \caption{RNF-05: Privacidad entre despachos}
  \label{tab:rnf-05}
  \begin{tabularx}{\linewidth}{@{}L{3.2cm}X@{}}
    \toprule
    \textbf{ID} & RNF-05 \\
    \midrule
    \textbf{Nombre} & Privacidad entre despachos \\
    \textbf{Descripción} & El sistema debe prohibir cualquier fuga de información inter-tenant, verificable mediante pruebas automatizadas de aislamiento. \\
    \textbf{Entrada} & Consultas con parámetros manipulados que incluyan identificadores de tenant ajeno. \\
    \textbf{Salida} & Respuesta vacía o error de autorización; nunca datos de otro tenant. \\
    \textbf{Actores involucrados} & Middleware de aislamiento; capa de acceso a datos; equipo de QA. \\
    \textbf{Criterio de aceptación} & Dado un conjunto de pruebas automatizadas de aislamiento ejecutadas contra el entorno de staging, entonces el 100 \% de los intentos de acceso a datos de otro tenant deben resultar en error de autorización. \\
    \bottomrule
  \end{tabularx}
\end{table}
...
\begin{table}[H]
  ...
  \begin{tabularx}{\linewidth}{@{}L{3.2cm}X@{}}
    ...
    \textbf{Salida} & Token RFC 3161 válido emitido por PSC autorizado, verificable de forma independiente. \\
    \textbf{Actores involucrados} & Sistema backend; PSC Cincel; equipo legal. \\
    \textbf{Criterio de aceptación} & Dado un token emitido por el sistema, cuando se verifica con OpenSSL usando el certificado público de la TSA de Cincel, entonces la verificación resulta exitosa y el timestamp corresponde a una fuente sincronizada con CENAM. \\
    \bottomrule
  \end{tabularx}
\end{table}
...
\begin{table}[H]
  ...
  \begin{tabularx}{\linewidth}{@{}L{0.22\linewidth}L{0.20\linewidth}L{0.14\linewidth}X@{}}
    ...
    Sellado de tiempo NOM-151 (PSC) & Cincel PSC -- Plan Start & \$0.00 MXN & Plan gratuito: 10 Certificados Smart (constancia NOM-151 + sello de tiempo + huella de auditoría). API Sandbox gratuita para desarrollo. \\
    ...
    Reserva de contingencia & Derivada del análisis de riesgos (sección \ref{sec:riesgos}) & \$35,578.13 MXN & VME total calculado en el análisis de riesgos. Representa el costo esperado ponderado por probabilidad de materialización. \\
    \bottomrule
  \end{tabularx}
\end{table}
...
\begin{table}[H]
  ...
  \begin{tabularx}{\linewidth}{@{}X r@{}}
    ...
    Costos de infraestructura y servicios & \$0.00 MXN \\
    Costo total sin contingencia & \$37,500.00 MXN \\
    Reserva de contingencia (VME total) & \$35,578.13 MXN \\
    Presupuesto total estimado & \$73,078.13 MXN \\
    \bottomrule
  \end{tabularx}
\end{table}

La reserva de contingencia representa el 94.9\,\% del costo base, lo cual refleja el alto riesgo técnico inherente a la integración de servicios criptográficos con tolerancia cero a fallos en un cronograma de 5 semanas de codificación. Este porcentaje es consistente con las recomendaciones del PMI para proyectos con incertidumbre técnica elevada.

Es importante señalar que los costos de infraestructura y servicios son cero exclusivamente en la fase de prototipo. Un despliegue en producción requeriría la contratación del plan Business de Cincel PSC para sellado de tiempo con volumen, infraestructura cloud dedicada con respaldos, y licencias de dominio y certificados de producción, rubros que quedan fuera del alcance de la presente estimación.
...
\begin{table}[H]
  \centering
  \caption{Ficha del riesgo RTC-01: Falla en integración con PSC (RFC 3161).}
  \label{tab:rtc-01}
  \begin{tabularx}{\linewidth}{@{}>{\bfseries}l X@{}}
    \toprule
    ID & RTC-01 \\
    Nombre & Falla en integración con PSC (RFC 3161) \\
    Cat. & Técnico \\
    Sev. & 6 -- Grave \\
    \midrule
    Declaración & \emph{Existe el riesgo de \textbf{que la integración con la API del PSC (Cincel) falle durante el flujo de sellado de tiempo}, debido a \textbf{incompatibilidades en el parseo del token ASN.1 o restricciones de red en el entorno de desarrollo}, lo que podría resultar en \textbf{la imposibilidad de emitir constancias NOM-151 válidas, bloqueando la validación completa del módulo de no repudio}.} \\
    Mitigación & Asignar las iteraciones iniciales a un Spike Arquitectónico para validar credenciales, red y parseo del token ASN.1 con el sandbox del PSC. \\
    Impacto & 2 -- Medio (30 h) \\
    VME & \$6,562.50 MXN \\
    Contingencia & Implementar un servicio Mock en Rust que simule las respuestas de la TSA, permitiendo avanzar con el desarrollo funcional mientras se estabiliza la conexión real. \\
    \bottomrule
  \end{tabularx}
\end{table}
...
\begin{table}[H]
  ...
  \begin{tabularx}{\linewidth}{@{}>{\bfseries}l X@{}}
    ...
    Sev. & 3 -- Moderado \\
    \midrule
    Declaración & \emph{Existe el riesgo de \textbf{que los expedientes cifrados se vuelvan irrecuperables de forma permanente}, debido a \textbf{la ausencia de un procedimiento formal de resguardo, rotación y recuperación de la clave maestra AES del tenant}, lo que podría resultar en \textbf{la pérdida total e irreversible de la información cifrada de todos los clientes del despacho, sin posibilidad de restauración}.} \\
...
\begin{table}[H]
  \centering
  \caption{Ficha del riesgo RLC-03: Fuga de datos inter-tenant (LFPDPPP).}
  \label{tab:rlc-03}
  \begin{tabularx}{\linewidth}{@{}>{\bfseries}l X@{}}
    \toprule
    ID & RLC-03 \\
    Nombre & Fuga de datos inter-tenant (LFPDPPP) \\
    Cat. & Legal \\
    Sev. & 3 -- Moderado \\
    \midrule
    Declaración & \emph{Existe el riesgo de \textbf{que datos confidenciales de un despacho sean accesibles por usuarios de otro tenant}, debido a \textbf{una falla en el middleware de aislamiento lógico multi-tenant que permita consultas sin restricción de \texttt{tenant\_id}}, lo que podría resultar en \textbf{una brecha de datos personales que viola el artículo 19 de la LFPDPPP, con exposición a sanciones administrativas y daño reputacional irreversible para el despacho}.} \\
    Mitigación & Inyectar restricción obligatoria de tenant en toda consulta relacional y verificar el aislamiento mediante pruebas automatizadas en cada iteración. \\
    Impacto & 3 -- Alto (50 h) \\
    VME & \$1,562.50 MXN \\
...
\begin{table}[H]
  \centering
  \caption{Ficha del riesgo RLC-04: Indisponibilidad del PSC (Cincel).}
  \label{tab:rlc-04}
  \begin{tabularx}{\linewidth}{@{}>{\bfseries}l X@{}}
    \toprule
    ID & RLC-04 \\
    Nombre & Indisponibilidad del PSC (Cincel) \\
    Cat. & Legal \\
    Sev. & 4 -- Significativo \\
    \midrule
    Declaración & \emph{Existe el riesgo de \textbf{que el módulo de sellado de tiempo quede fuera de servicio durante ventanas críticas de firma}, debido a \textbf{una interrupción no planificada en la infraestructura del Prestador de Servicios de Certificación seleccionado}, lo que podría resultar en \textbf{el incumplimiento de plazos procesales del CNPP que dependen de la emisión oportuna de la constancia NOM-151, con potencial afectación al valor probatorio de los documentos}.} \\
    Mitigación & Diseñar el módulo de sellado con una interfaz abstracta que permita configurar un PSC alternativo (SeguriData, CECOBAN) sin modificar la lógica central del sistema. \\
    Impacto & 2 -- Medio (20 h) \\
    VME & \$3,125.00 MXN \\
    Contingencia & Encolar las solicitudes de sellado fallidas y reintentar automáticamente al restablecerse el servicio; notificar al Litigante responsable sobre el retraso en la constancia. \\
    \bottomrule
  \end{tabularx}
\end{table}
...
\begin{table}[H]
  ...
  \begin{tabularx}{\linewidth}{@{}>{\bfseries}l X@{}}
    ...
    Sev. & 4 -- Significativo \\
    \midrule
    Declaración & \emph{Existe el riesgo de \textbf{que el esfuerzo dedicado a interfaces gráficas consuma tiempo crítico de las semanas 8 y 9}, debido a \textbf{la tendencia a invertir iteraciones en refinamiento visual más allá de lo planificado, en detrimento de los módulos de seguridad de mayor criticidad}, lo que podría resultar en \textbf{el desplazamiento de las Iteraciones 8 y 9 (cifrado autenticado y no repudio) hacia fuera del cronograma, comprometiendo los entregables criptográficos centrales del prototipo}.} \\
    Mitigación & Congelar las historias de usuario visuales y utilizar bibliotecas de componentes preconstruidas para la interfaz, prohibiendo el diseño a medida en la Fase II. \\
    Impacto & 2 -- Medio (20 h) \\
    VME & \$3,125.00 MXN \\
    Contingencia & Recortar características no críticas (RF-19 Dashboard Ejecutivo o reportes visuales) para canalizar el esfuerzo íntegramente a los módulos de no repudio y cifrado. \\
    \bottomrule
  \end{tabularx}
\end{table}
...
\begin{table}[H]
  ...
  \begin{tabularx}{\linewidth}{@{}l X L{0.13\linewidth}L{0.13\linewidth}L{0.16\linewidth}r@{}}
    ...
    \midrule
    \textbf{RTC-01} & Falla en integración con PSC (RFC 3161) & 3 Alto (70\,\%) & 2 Medio (30 h) & \textbf{6 -- Grave} & \$6,562.50 MXN \\
    ...
    \textbf{RLC-03} & Fuga de datos inter-tenant (LFPDPPP) & 1 Bajo (10\,\%) & 3 Alto (50 h) & \textbf{3 -- Moderado} & \$1,562.50 MXN \\
    \textbf{RLC-04} & Indisponibilidad del PSC (Cincel) & 2 Medio (30\,\%) & 2 Medio (20 h) & \textbf{4 -- Significativo} & \$3,125.00 MXN \\
...
\textbf{Recomendación:} El éxito del prototipo radica en su validez como evidencia técnica, no en su amplitud estética ni de reportes. Todo el esfuerzo de exploración de la Fase I (semanas 1 a 5) debe destinarse a resolver los Spikes correspondientes al consumo de la API del PSC y al cifrado bidireccional de AES-256-GCM. Se recomienda no avanzar hacia el desarrollo de vistas web hasta haber comprobado técnicamente la validez del sellado de tiempo de prueba.
...
\begin{table}[H]
  ...
  \begin{tabularx}{\linewidth}{@{}lX L{0.30\linewidth}@{}}
    ...
    \midrule
    RTC-01 & Falla en integración con PSC (RFC 3161) & OE-3 \\
    ...
    RLC-03 & Fuga de datos inter-tenant (LFPDPPP) & OE-4 (residual en single-tenant) \\
    RLC-04 & Indisponibilidad del PSC (Cincel) & OE-3 \\
...
\begin{table}[H]
  ...
  \begin{tabularx}{\linewidth}{@{}C{0.18\linewidth}L{0.34\linewidth}X@{}}
    ...
    \midrule
    1 (más alta) & Seguridad & RNF-01, RNF-02, RNF-03, RNF-09 \\
    2 & Integridad y trazabilidad & RNF-04, RNF-08 \\
    3 & Evolvabilidad & Sustitución de PSC, BD, SMTP \\
    4 & Testabilidad & XP/TDD (§3.6) \\
...
Los servicios externos se modelan como entidades de la zona no confiable a las que el sistema invoca explícitamente con canales seguros: la integración con Cincel PSC para sellado de tiempo (RFC 3161) utiliza TLS mutuo más verificación de la firma del sello recibido; la integración con el proveedor SMTP utiliza TLS más autenticación SASL. En ningún caso se confía en estos servicios para operaciones de seguridad sin verificación local del resultado: por ejemplo, el sello de tiempo recibido de Cincel se verifica criptográficamente contra la cadena de certificados del PSC antes de ser aceptado e incorporado al expediente del documento firmado.
...
\begin{table}[H]
  ...
  \begin{tabularx}{\linewidth}{@{}L{0.42\linewidth}X@{}}
    ...
    \midrule
    Bounded Context «Identidad y Acceso» & CU-01..04, RNF-01, RNF-02, RNF-09 \\
    ...
    Bounded Context «Operaciones y Auditoría» & CU-15..17, RNF-04, RNF-08 \\
    ...
    Pipeline Tower middleware & RNF-09 (autenticación), seguridad cross-cutting \\
    Modelo de fronteras de confianza (§3.7.9) & RNF-01, RNF-02, RNF-03, RNF-09 \\
    Subsistema observabilidad (tracing) & RNF-08 \\
    \bottomrule
  \end{tabularx}
\end{table}
...
La bitácora de auditoría (audit\_log) implementa un mecanismo de \textbf{encadenamiento SHA-256}: cada registro incluye un campo sha256\_chain que contiene el hash (véase §2.3.1) del registro anterior concatenado con los datos del registro actual. Esto crea una cadena verificable de integridad que permite detectar cualquier manipulación retroactiva, cumpliendo con el Art. 33 del Código de Comercio (retención mínima de 10 años).

\subsubsection{Decisión de diseño: Single-Tenant}

El sistema se diseña para un solo despacho legal (single-tenant), eliminando la complejidad de aislamiento multi-tenant descrita en RNF-01 y RNF-05 del protocolo original. Esta simplificación es coherente con el alcance del prototipo y reduce significativamente la superficie de ataque.
...
La elección de una CA interna sobre la e.firma del SAT o un PSC comercial responde a múltiples consideraciones. En primer lugar, las firmas generadas con certificados de la CA interna son matemáticamente idénticas a las generadas con certificados de la e.firma o de un PSC: mismo algoritmo (RSA-3072 + SHA-256), misma estructura X.509 v3, mismo proceso de verificación. Lo único que cambia es el emisor del certificado. La demostración de los servicios de integridad, autoría y no repudio es, por tanto, completamente válida con esta elección. En segundo lugar, el valor probatorio del flujo de firma se sostiene sobre el sello de tiempo RFC 3161 \cite{adams2001} emitido por Cincel bajo NOM-151 \cite{nom151_2016}, no sobre la cadena de confianza del certificado del firmante; el sello acredita de forma independiente la existencia e integridad del documento en un momento específico, que es el elemento probatorio más difícil de sustituir.
...
El sellado de tiempo se delega en Cincel, PSC autorizado por la Secretaría de Economía para la emisión de Sellos Digitales de Tiempo y constancias de conservación conforme a la NOM-151-SCFI-2016 \cite{comparasoftware2026}. El flujo opera conforme al protocolo RFC 3161 \cite{adams2001}: se calcula el hash SHA-256 del documento firmado, se envía a la API REST de Cincel \cite{cincel2024}, y se recibe una respuesta TimeStampResp en formato ASN.1 DER que se almacena como artefacto independiente (.tsr) vinculado al documento en la base de datos. El módulo de sellado abstrae al proveedor detrás de una interfaz interna, permitiendo sustituir Cincel por SeguriData o CECOBAN sin modificar la lógica central, siempre que el proveedor alternativo implemente RFC 3161.
...
La selección de Cincel como proveedor de sellado de tiempo obedece a tres factores. Primero, Cincel es un PSC acreditado por la Secretaría de Economía para la emisión de Sellos Digitales de Tiempo (SDT) y constancias de conservación de mensajes de datos (CCMD), con acreditación publicada en el Diario Oficial de la Federación el 24 de noviembre de 2023 \cite{comparasoftware2026}. Segundo, Cincel expone un entorno de sandbox sin costo que permite obtener sellos de prueba durante la fase de desarrollo sin contratar un plan comercial, lo que es determinante para el cronograma de diez semanas del prototipo. Tercero, su API REST v3 \cite{cincel2024} proporciona una interfaz moderna basada en JSON: se envía el hash SHA-256 del documento en formato hexadecimal y se recibe un enlace al archivo ASN.1 (NOM-151) en formato DER conforme a RFC 3161 \cite{adams2001}. Si el PSC tarda más de 100 milisegundos en generar el sello, Cincel lo procesa de forma asíncrona y recomienda reintentar tras aproximadamente un minuto, lo que el prototipo maneja con un mecanismo de reintento configurable.

El sello de tiempo se almacena como la respuesta cruda TimeStampResp en formato ASN.1 DER (extensión .tsr) en lugar de empaquetarlo en una estructura CAdES-T o embebido en un PDF (PAdES-T). Esta decisión se justifica por tres razones. Primera, con artefactos independientes cada componente del flujo criptográfico —documento, firma, sello, certificado— puede inspeccionarse y verificarse por separado con herramientas estándar; el comando openssl ts -verify opera directamente sobre archivos .tsr sin necesidad de parsear contenedores CMS ni estructuras PDF. Segunda, CAdES-T requeriría construir estructuras CMS con atributos no firmados y PAdES-T requeriría manipular los mecanismos internos del formato PDF (ByteRange, signature dictionaries, incremental saves), tareas de ingeniería de formatos que consumirían tiempo del cronograma sin aportar al objetivo criptográfico del Trabajo Terminal. Tercera, el valor legal es idéntico independientemente del contenedor: el sello de tiempo que aporta valor probatorio bajo la NOM-151 \cite{nom151_2016} es el TimeStampToken RFC 3161 emitido por Cincel, y ese token es criptográficamente el mismo ya sea almacenado como .tsr, embebido en CAdES-T o dentro de PAdES-T. La norma no prescribe un formato de contenedor específico.

El flujo de verificación completo del sello de tiempo procede en cuatro pasos: (1) se parsea el TimeStampResp para extraer el TimeStampToken; (2) se verifica que el campo messageImprint.hashedMessage coincida con el hash SHA-256 recalculado del documento original; (3) se verifica la firma de la TSA de Cincel sobre el token; y (4) se verifica el certificado de la TSA contra la cadena de confianza del PSC emisor hasta la ACR2-SE \cite{economia_acr2se}. Con OpenSSL, este proceso completo se ejecuta con un solo comando: openssl ts -verify -data documento.pdf -in sello.tsr -CAfile cincel\_ca.pem. Un despliegue productivo podría migrar a CAdES-T o PAdES-T para ofrecer un paquete autocontenido más conveniente para el usuario final, sin cambiar el token subyacente ni el flujo de obtención del sello, dado que el TimeStampToken es el mismo objeto ASN.1 en cualquier contenedor.
...
Las contraseñas de los usuarios se almacenan hasheadas con Argon2id (RFC 9106) \cite{biryukov2015,biryukov2021}, la variante híbrida recomendada por OWASP \cite{owasp_pwstorage2024} y NIST SP 800-63B Rev 4 \cite{nist2024} para nuevas implementaciones. Los parámetros adoptados son: memoria m = 46 MiB, iteraciones t = 1, paralelismo p = 1, sal de 16 bytes y hash de salida de 32 bytes, correspondientes a la configuración recomendada de OWASP, calibrada para producir un tiempo de hash de 0.5--1.0 segundo por intento legítimo. Estos parámetros deben verificarse empíricamente en el hardware de despliegue antes de la puesta en producción.
...
La firma generada por el prototipo constituye una firma electrónica simple conforme al artículo 89 del Código de Comercio \cite{ccom2018}, dado que los certificados no provienen de un PSC autorizado (artículo 97, requisito 5). No obstante, su valor probatorio se sustenta en tres pilares: (1) la admisibilidad de la firma simple como prueba, ya que el artículo 89 no condiciona la admisibilidad a la intervención de un PSC; (2) la cláusula de equivalencia funcional del artículo 100, que permite argumentar la fiabilidad del método basándose en la robustez de estándares reconocidos internacionalmente (RSA-3072 conforme a NIST SP 800-57 \cite{barker2020}, SHA-256 conforme a FIPS 180-4 \cite{nist2015fips1804}); y (3) el sello de tiempo NOM-151 de Cincel, que proporciona fecha cierta, integridad verificada por un tercero de confianza autorizado y cumplimiento del artículo 93 bis. La distinción práctica respecto de una firma avanzada radica en la carga de la prueba: quien invoca la firma simple debe acreditar la fiabilidad del método, mientras que la firma avanzada goza de presunción legal. En un despliegue productivo, la sustitución de la CA interna por un PSC autorizado convertiría la firma en avanzada sin cambios arquitectónicos, dado que el módulo de firma acepta cualquier certificado X.509 RSA + SHA-256.
