...
\begin{table}[H]
  ...
  \begin{tabularx}{\linewidth}{@{}l X@{}}
    ...
    \texttt{issue-declaration-cert.sh} & Emite una hoja para declaraciones personales con usos de firma y sin EKU de autenticación o correo. \\
...
Produce la lista de certificados revocados en formato PEM a partir de la base de datos de la CA. Cada CRL es válida siete días (\texttt{default\_crl\_days = 7} en \texttt{openssl.cnf}), por lo que debe regenerarse antes de caducar y después de cada revocación; el validador del prototipo trata una CRL vencida como error duro, nunca como un pase permisivo. La configuración incorpora \texttt{authorityKeyIdentifier}; OpenSSL añade el número de CRL a partir de \texttt{crlnumber}. Al terminar, el script reporta las fechas \texttt{lastUpdate} y \texttt{nextUpdate} de la lista recién emitida y cuenta los certificados revocados que contiene. Las declaraciones de participantes requieren además publicar esa CRL en PostgreSQL.
...
\input{chapters/anexo-d-declaraciones}
...
\begin{enumerate}
  ...
  \item Emitir el certificado con \texttt{issue-cert.sh} para el uso ordinario o \texttt{issue-declaration-cert.sh} para la declaración personal interna, proporcionando el nombre sintético del titular.
  ...
  \item \texttt{gen-crl.sh}, después de cada revocación y antes de caducar, para que los verificadores dispongan de una lista vigente.
  \item Publicar raíz y CRL con \texttt{credential-trust publish} cuando se usa la admisión de declaraciones personales; las publicaciones posteriores conservan la raíz y exigen la revisión esperada.
\end{enumerate}
