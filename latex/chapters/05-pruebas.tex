...
\begin{table}[H]
  ...
  \begin{tabularx}{\linewidth}{@{}l X R{2.6cm} R{2.9cm} R{2.0cm}@{}}
    ...
    \texttt{web} & contrato HTTP de salud con Axum & 1 & 0 & 1 \\
    \midrule
    \textbf{Total} & & \textbf{198} & \textbf{169} & \textbf{367} \\
    \bottomrule
  \end{tabularx}
...
La \cref{tab:pruebas-cobertura} presenta la cobertura de líneas final por crate, medida con \texttt{cargo-llvm-cov} sobre la suite completa y reproducida en el corte del 17 de agosto de 2026. El umbral del 90\,\% que exige el requerimiento RNF-13 se aplica en integración continua a los tres crates con lógica criptográfica mediante \texttt{scripts/coverage-gate.sh}, y los tres lo superan. El crate \texttt{bin} se reporta sin umbral: es la interfaz de línea de comandos y la raíz de composición, cuyas líneas no cubiertas corresponden mayormente al formateo de salidas y a ramas de error del arranque; aun así queda en 88.1\,\%. Parte del margen sobre el umbral proviene de pruebas dedicadas a rutas de error de los adaptadores ---una bitácora cuya ruta es un directorio, una línea almacenada con timestamp imparseable, respuestas del proveedor de sellado sin token o con base64 inválido, material PEM corrupto en el firmador y el verificador---, de modo que los caminos de fallo están tan ejercitados como los de éxito.
...
\begin{longtable}{@{}l L{0.40\linewidth} L{0.36\linewidth} L{0.10\linewidth}@{}}
  ...
  \addlinespace
  OE-5 & Argon2id calibrado a 0.5--1.0 s por intento en el hardware de referencia ($m = 256$~MiB, $t = 4$, $p = 1$). & Cinco corridas en AMD Ryzen 7 7730U promediaron 607.8 ms y emitieron veredicto dentro de la banda; repetir si cambia el hardware de despliegue (\cref{sec:pruebas-argon2}). & Cumplido* \\
  \addlinespace
...
El balance debe leerse con la misma franqueza con que se construyó la tabla. Los objetivos OE-4 y OE-6 se cumplen en todos sus subcriterios. OE-3 queda parcialmente cumplido porque su criterio externo fue excluido de esta entrega; OE-5 conserva dos pendientes operativos precisos y acotados:
...
\begin{itemize}
  ...
  \item \textbf{Banda de calibración de Argon2id (OE-5).} El hardware de referencia quedó dentro de banda con $m = 256$~MiB, $t = 4$, $p = 1$ y una media de 607.8 ms. El asterisco de la tabla recuerda que un hardware de despliegue distinto exige repetir la medición (\cref{sec:pruebas-argon2}).
\end{itemize}

En síntesis, la validación deja los servicios criptográficos del prototipo respaldados por 366 pruebas en verde con una cobertura del 93.6\,\% del workspace, vectores oficiales reproducidos para cada primitivo que los publica, detección total en las campañas de mutación, y verificación independiente bidireccional con OpenSSL 3.5.7 documentada en una transcripción versionada. También deja explícito que la evidencia externa de un PSC no forma parte de esta entrega y que la TSA local no se presenta como equivalente jurídico. Los pendientes técnicos que siguen son la construcción de la superficie web con persistencia y RBAC; la API ya cuenta con un primer adaptador HTTP verificable. Las conclusiones generales del trabajo se presentan en el Capítulo~\ref{cap:conclusiones}.
