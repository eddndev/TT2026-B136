...
\begin{table}[H]
  ...
  \begin{tabularx}{\linewidth}{@{}l X R{2.6cm} R{2.9cm} R{2.0cm}@{}}
    ...
    \midrule
    \texttt{domain} & hashers falsos deterministas, sin criptografía real & 69 & --- & 69 \\
    \texttt{application} & mocks \texttt{mockall} de los puertos & 39 & 36 & 75 \\
    \texttt{infrastructure} & criptografía real y herramientas del sistema & 86 & 81\textsuperscript{a} & 167 \\
    \texttt{bin} & el binario compilado como subproceso & 3 & 52 & 55 \\
    \texttt{web} & (sin pruebas: crate vacío) & 0 & 0 & 0 \\
    \midrule
    \textbf{Total} & & \textbf{197} & \textbf{169} & \textbf{366} \\
    \bottomrule
  \end{tabularx}
...
\begin{table}[H]
  ...
  \begin{tabularx}{\linewidth}{@{}l X C{2.2cm} C{3.2cm}@{}}
    ...
    \multicolumn{2}{@{}l}{Pruebas transversales (CLI, errores, fitness functions)} & 11 & \\
    \bottomrule
  \end{tabularx}
...
El diseño fijó los parámetros de Argon2id en $m = 46$~MiB, $t = 1$, $p = 1$ ---la configuración de primera elección publicada por OWASP y segunda recomendación de RFC 9106 \cite{biryukov2021,owasp_pwstorage2024}--- y exigió verificar empíricamente que el costo por intento quede entre 0.5 y 1.0 segundos en el hardware de despliegue (\cref{sec:autenticacion}). El caso de uso de calibración implementado para ese fin (subcomando \texttt{auth calibrate}) promedia cinco corridas de hash sobre una muestra fija y clasifica la media contra la banda objetivo inclusiva. Ejecutado en el hardware de desarrollo, reportó una media de 37.8 milisegundos sobre las cinco corridas ---la corrida capturada en la transcripción de la demostración---, es decir, un veredicto por debajo de la banda.

La lectura honesta de este resultado es la siguiente. La banda de 0.5 a 1.0 segundos está definida para el hardware de despliegue, no para la máquina de desarrollo en que se midió; el resultado no acredita ni desacredita el criterio del objetivo OE-5, sino que demuestra que el instrumento de medición funciona y que los parámetros mínimos recomendados por OWASP son insuficientes en hardware moderno rápido si se desea alcanzar la banda. En el despliegue deberá repetirse la calibración con el mismo comando y elevarse los parámetros ---iteraciones o memoria--- hasta que la media entre en banda; los parámetros están declarados como constantes en un único punto del adaptador precisamente para que ese ajuste sea localizado. El resto del servicio no depende de la banda: el formato PHC con los parámetros registrados, la coincidencia y no coincidencia de contraseñas, el tratamiento del hash corrupto como error y la ausencia de la contraseña en los diagnósticos están fijados por pruebas que pasan en cualquier hardware.
...
La evidencia operativa de este enfoque es la demostración de extremo a extremo \texttt{scripts/\allowbreak demo.sh} (\cref{sec:impl-cli}), cuya transcripción íntegra ---capturada el 12 de julio de 2026 sobre OpenSSL 3.5.7 y re-ejecutada sin errores con posterioridad--- está versionada en \texttt{docs/demo-transcript.md}. La demostración no es una vitrina: cada caso negativo se ejecuta bajo un auxiliar que aborta el guion si el comando \emph{no} es rechazado, de modo que los rechazos esperados son verificaciones reales. Los extractos siguientes reproducen tres momentos de esa transcripción; se abrevian las rutas del directorio temporal y del binario, sin alterar ninguna salida. El primero es el reporte de la verificación integral con los cuatro componentes en verde:
...
\begin{longtable}{@{}l L{0.40\linewidth} L{0.36\linewidth} L{0.10\linewidth}@{}}
  ...
  \addlinespace
  OE-3 & Tasa de verificación exitosa $\geq$ 99\,\% sobre sellos emitidos por el sandbox de Cincel. & No medible aún: la campaña de $N \geq 100$ sellos contra el sandbox del PSC está pendiente de credenciales; la prueba de humo está escrita y lista para ejecutarse. & Diferido \\
  \addlinespace
  ...
  \addlinespace
  OE-5 & Argon2id calibrado a 0.5--1.0 s por intento en el hardware de despliegue ($m = 46$~MiB, $t = 1$, $p = 1$). & Parámetros fijados por prueba y mecanismo de calibración operativo; en el hardware de desarrollo la media fue de 37.8 ms, fuera de banda; la calibración en el hardware de despliegue queda pendiente (\cref{sec:pruebas-argon2}). & Parcial \\
  \addlinespace
  ...
  \addlinespace
  OE-5 & Control de acceso RBAC con cuatro roles diferenciados (Owner, Litigante, Paralegal, Cliente). & Diferido junto con la capa web: no existe aún superficie multiusuario que autorizar. & Diferido \\
  \addlinespace
  OE-6 & Cobertura de pruebas $\geq$ 80\,\% en los módulos criptográficos. & Entre 93.5 y 96.2\,\% por crate criptográfico; el umbral aplicado en CI es el 90\,\% de RNF-13, más estricto que el criterio. & Cumplido \\
  \addlinespace
...
El balance debe leerse con la misma franqueza con que se construyó la tabla. Los objetivos OE-4 y OE-6 se cumplen en todos sus subcriterios. Los objetivos OE-3 y OE-5 quedan parcialmente cumplidos, cada uno con un pendiente preciso y acotado:

\begin{itemize}
  \item \textbf{Campaña contra el sandbox del PSC (OE-3).} La autoridad de sellado local demuestra el flujo RFC 3161 completo ---solicitud, emisión, token DER, verificación en dos etapas y aceptación por herramientas independientes---, pero el criterio de tasa de verificación $\geq$ 99\,\% se enuncia sobre sellos del sandbox de Cincel y, sin credenciales del proveedor, no es medible todavía. El pendiente está acotado arquitectónicamente (\cref{sec:impl-spike-tsa}): el adaptador remoto existe y está probado contra un stub que ejercita su contrato completo, la prueba de humo contra el sandbox real está escrita ---es la única prueba ignorada de la suite---, y la campaña de $N \geq 100$ sellos con registro de la tasa es la primera actividad programada al obtener las credenciales.
  \item \textbf{RBAC de cuatro roles (OE-5).} El control de acceso por roles pertenece a la superficie web multiusuario, cuya construcción quedó diferida a la siguiente fase junto con el crate \texttt{web} (\cref{sec:impl-workspace}); en la interfaz administrativa de línea de comandos entregada no hay sesiones ni roles que diferenciar. Los cimientos de autenticación sobre los que se montará ---hash de contraseñas, segundo factor y códigos de recuperación--- están implementados y probados; la exigencia de un solo uso por código TOTP y la limitación de intentos, que requieren un almacén de estado por cuenta, quedaron registradas como diferidas en el mismo movimiento (\cref{sec:impl-divergencias}).
  \item \textbf{Banda de calibración de Argon2id (OE-5).} La banda de 0.5 a 1.0 segundos no aplica al hardware de desarrollo, donde la media medida fue de 37.8 ms; el criterio solo podrá declararse cumplido tras recalibrar en el hardware de despliegue elevando los parámetros hasta entrar en banda, con el instrumento ya implementado y ejercitado para ese propósito (\cref{sec:pruebas-argon2}).
\end{itemize}

En síntesis, la validación de esta fase deja los servicios criptográficos del prototipo respaldados por 365 pruebas en verde con una cobertura del 93.6\,\% del workspace, vectores oficiales reproducidos para cada primitivo que los publica, detección total en las campañas de mutación, y verificación independiente bidireccional con OpenSSL 3.5.7 documentada en una transcripción versionada; y deja explícitos, con su causa y su vía de cierre, los tres pendientes que la siguiente fase debe saldar. Las conclusiones generales del trabajo se presentan en el Capítulo~\ref{cap:conclusiones}.
