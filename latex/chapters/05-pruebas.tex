...
\begin{table}[H]
  ...
  \begin{tabularx}{\linewidth}{@{}l X R{2.6cm} R{2.9cm} R{2.0cm}@{}}
    ...
    \texttt{application} & mocks \texttt{mockall} de los puertos & 39 & 43 & 82 \\
    \texttt{infrastructure} & criptografía real y herramientas del sistema & 86 & 87\textsuperscript{a} & 173 \\
    \texttt{bin} & el binario compilado como subproceso & 3 & 52 & 55 \\
    \texttt{web} & contrato HTTP con workflow inyectado & 1 & 3 & 4 \\
    \midrule
    \textbf{Total} & & \textbf{198} & \textbf{185} & \textbf{383} \\
    \bottomrule
  \end{tabularx}
...
\begin{table}[H]
  ...
  \begin{tabularx}{\linewidth}{@{}l X C{2.2cm} C{3.2cm}@{}}
    ...
    \multicolumn{2}{@{}l}{Transversales (workflow, HTTP, CLI y fitness)} & 27 & \\
    \bottomrule
  \end{tabularx}
...
La evidencia operativa de este enfoque comprende dos demostraciones de extremo a extremo. \texttt{scripts/\allowbreak demo.sh} (\cref{sec:impl-cli}), cuya transcripción íntegra ---capturada el 12 de julio de 2026 sobre OpenSSL 3.5.7 y re-ejecutada sin errores con posterioridad--- está versionada en \texttt{docs/demo-transcript.md}, recorre la CLI. \texttt{scripts/\allowbreak api-demo.sh} levanta la aplicación en un puerto efímero, recorre carga, sellado, verificación, exportación y auditoría a través de HTTP, comprueba que el texto claro no aparece en el repositorio y valida el ZIP descargado con OpenSSL y \texttt{unzip}; las variables de Cincel se eliminan expresamente del entorno. Las demostraciones no son vitrinas: cada condición esperada aborta el guion si no se cumple. Los extractos siguientes reproducen tres momentos de la primera transcripción; se abrevian las rutas del directorio temporal y del binario, sin alterar ninguna salida. El primero es el reporte de la verificación integral con los cuatro componentes en verde:
...
\begin{longtable}{@{}l L{0.40\linewidth} L{0.36\linewidth} L{0.10\linewidth}@{}}
  ...
  \addlinespace
  OE-5 & Control de acceso RBAC con cuatro roles diferenciados (Owner, Litigante, Paralegal, Cliente). & La superficie HTTP documental existe, pero todavía no autentica sesiones ni aplica permisos; \texttt{X-Actor} es solo una etiqueta de auditoría. & Diferido \\
  \addlinespace
  OE-6 & Cobertura de pruebas $\geq$ 80\,\% en los módulos criptográficos. & Entre 93.4 y 96.2\,\% por crate criptográfico; el umbral aplicado en CI es el 90\,\% de RNF-13, más estricto que el criterio. & Cumplido \\
  \addlinespace
...
\begin{itemize}
  ...
  \item \textbf{RBAC de cuatro roles (OE-5).} La superficie HTTP documental ya existe, pero opera como aplicación local: \texttt{X-Actor} etiqueta la auditoría y no prueba identidad ni permisos. Los cimientos de autenticación sobre los que se montarán las sesiones ---hash de contraseñas, segundo factor y códigos de recuperación--- están implementados y probados; JWT, RBAC, la exigencia de un solo uso por código TOTP y la limitación de intentos requieren un almacén persistente por cuenta y permanecen diferidos (\cref{sec:impl-divergencias}).
...
En síntesis, la validación deja los servicios criptográficos y la rebanada HTTP local respaldados por 382 pruebas en verde con una cobertura del 91.9\,\% del workspace, vectores oficiales reproducidos para cada primitivo que los publica, detección total en las campañas de mutación, y verificación independiente bidireccional con OpenSSL 3.5.7. También deja explícito que la evidencia externa de un PSC no forma parte de esta entrega y que la TSA local no se presenta como equivalente jurídico. Los pendientes técnicos son sesiones y RBAC, persistencia PostgreSQL/Redis y la interfaz de usuario; ya no incluyen la orquestación documental ni su contrato HTTP básico. Las conclusiones generales del trabajo se presentan en el Capítulo~\ref{cap:conclusiones}.
