...
El corte reproducido del 12 de septiembre de 2026 aprobó \textbf{519 pruebas, sin fallos y con una ignorada}, según el informe versionado \rutarepo{docs/verification-report.md}. La prueba ignorada corresponde al proveedor externo de sellado; las ejecuciones ordinarias emplean un sustituto HTTP local y la TSA OpenSSL del prototipo, sin consumir el servicio comercial. El resultado incorpora autorización documental por expediente, persistencia y auditoría transaccionales, migración de archivos cifrados y restauración. PostgreSQL y Redis fueron procesos reales y desechables, con bases separadas para identidad, expedientes y documentos. Los porcentajes de cobertura de este capítulo corresponden a esa ejecución; la calibración de Argon2id y los extractos de una transcripción anterior se identifican como evidencia histórica.
...
El criterio del objetivo OE-6 exige que la matriz contenga, por lo menos, un caso positivo y uno negativo por caso de uso. La \cref{tab:pruebas-casos} nombra un caso representativo de cada signo por servicio; como documenta el inventario, todos los servicios poseen varios casos negativos adicionales ---la suite incluye rechazos tipados por evidencia inválida, falta de permisos y fallos de persistencia---. El inventario de las 520 funciones, agrupadas por archivo con casos representativos, se entrega en el Anexo~\ref{anexo:matriz}.
...
\begin{longtable}{@{}l L{0.40\linewidth} L{0.36\linewidth} L{0.10\linewidth}@{}}
  ...
  \addlinespace
  OE-6 & Cobertura de pruebas $\geq$ 80\,\% en los módulos criptográficos. & Entre 92.4 y 96.8\,\% por crate criptográfico; el umbral aplicado en CI es el 90\,\% de RNF-13, más estricto que el criterio. & Cumplido \\
  \addlinespace
  ...
  \addlinespace
  OE-6 & Matriz de pruebas funcionales con, por lo menos, un caso positivo y uno negativo por caso de uso. & \Cref{tab:pruebas-casos} por servicio; inventario por archivo y casos representativos en el Anexo~\ref{anexo:matriz}. & Cumplido \\
\end{longtable}
...
El corte respalda el backend criptográfico y HTTP con 519 pruebas ejecutadas, una cobertura global de 90.7\,\%, vectores oficiales y verificaciones independientes. Los ensayos documentan autorización por expediente, atomicidad, migración y restauración sobre datos generados. No constituyen una campaña de carga de producción, una evaluación de usabilidad ni evidencia de cumplimiento jurídico. Quedan pendientes consultas y versiones documentales, participantes procesales, interfaz de usuario, TLS, pooling, apagado ordenado y revisión de la firma RSA expuesta por HTTP. La cabeza de auditoría carece de anclaje externo; una cadena válida aislada no prueba que no se haya truncado su extremo. La TSA local no equivale a una constancia de un PSC autorizado. Las conclusiones generales se presentan en el Capítulo~\ref{cap:conclusiones}.
