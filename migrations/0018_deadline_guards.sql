-- Human V1/V2 writes retain exact history; technical writes require a separate authorized path.
DO $install$ BEGIN
...
BEGIN
    ...
    IF NEW.action='reevaluate' OR (tracked AND receipt->'author'->>'kind' IS DISTINCT FROM 'user') THEN
        RAISE EXCEPTION 'technical deadline writes require an authorized durable worker' USING ERRCODE='42501'; END IF;
    IF tracked THEN
