...
use application::{hearing_results::*, identity::Principal};
use domain::{
...
fn committed_capture_time_is_observed_after_waiting_for_the_shared_audit_lock() {
    ...
    let interleaved = hearing_result_revalidation_support::interleaved(store, move || {
    ...
    let workflow = HearingResultService::new(
        interleaved,
    ...
    assert!(
        observed,
        "prepared hearing result did not wait for the shared audit lock"
    );
    ...
    assert_eq!(captured.snapshot.recorded_administration_revision.get(), 1);
}
