...
pub(super) fn validate<C: GenericClient>(client: &mut C) -> Result<(), ApplicationError> {
    for table in TABLES {
        ordinary(
            client,
            table,
            "procedural_resource_lock",
            "lock_procedural_resource_history()",
            6,
        )?;
        ordinary(
            client,
            table,
            "procedural_resource_immutable",
            "preserve_procedural_resource_history()",
            58,
        )?;
        let expected: i64 = if table == TABLES[2] { 3 } else { 2 };
    ...
    ordinary(
        client,
        TABLES[2],
        "procedural_resource_sequence",
        "enforce_procedural_resource_sequence()",
        7,
    )
}
