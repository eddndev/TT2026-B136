...
pub(super) fn validate<C: GenericClient>(client: &mut C) -> Result<(), ApplicationError> {
    ...
    key_constraint(client, TABLES[1], "p", &["calendar_id", "revision"], false)?;
    ...
    for (table, target, names, targets, deferred) in [
        ...
        (TABLES[1], TABLES[0], vec!["calendar_id"], vec!["id"], false),
    ...
    for (table, count) in [(TABLES[0], 3_i64), (TABLES[1], 15_i64)] {
