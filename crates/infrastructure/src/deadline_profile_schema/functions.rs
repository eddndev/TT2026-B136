    ...
    for sql in SQL {
        for definition in sql.split("CREATE OR REPLACE FUNCTION ").skip(1) {
            ...
            if declaration.starts_with(&format!("{name}(")) {
                return rest
                    .split_once("$$;")
                    .map(|(body, _)| body.replace("%1$I", schema));
            }
        }
    }
    None
