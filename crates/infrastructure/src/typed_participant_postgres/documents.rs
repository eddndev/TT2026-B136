...
use application::typed_participants::{ParticipantEvidenceLocator, StageSupportReadLimits};
...
pub(super) fn require_scope(
    ...
    case: CaseId,
    supports: &[ParticipantEvidenceLocator],
...
pub(super) fn load(
    tx: &mut Transaction<'_>,
    case: CaseId,
    support: &ParticipantEvidenceLocator,
    ...
    let vault_size =
        usize::try_from(probe.get::<_, i64>("vault_size")).map_err(|_| inconsistent())?;
    let header: Vec<u8> = probe.get("header");
    limits.vault().inspect_header(&header, vault_size)?;
    let evidence_size =
        usize::try_from(probe.get::<_, i64>("evidence_size")).map_err(|_| inconsistent())?;
    ...
    let maximum = i64::try_from(limits.vault().max_vault_bytes()).map_err(|_| inconsistent())?;
    let evidence_maximum =
        i64::try_from(limits.max_evidence_json_bytes()).map_err(|_| inconsistent())?;
    // The final statement repeats every allocation bound in its own snapshot.
    let row=tx.query_opt("SELECT id,version,name,digest,vault,evidence::text AS evidence_text FROM documents WHERE id=$1 AND case_id=$2 AND version=$3 AND octet_length(vault)::bigint BETWEEN 97 AND $4 AND substring(vault FROM 1 FOR 9)=$5 AND octet_length(name)<=128 AND octet_length(digest)=32 AND COALESCE(octet_length(evidence::text),0)::bigint<=$6",&[&id,&case,&version,&maximum,&header,&evidence_maximum]).map_err(port)?.ok_or(ApplicationError::ParticipantSupportChanged)?;
    ...
    let mut record = DocumentRecord::pending(
        ...
        DocumentVersion::new(
            u32::try_from(row.get::<_, i64>("version")).map_err(|_| inconsistent())?,
        )
