use super::{incomplete, port, FUNCTIONS};
...
pub(super) fn validate<C: GenericClient>(client: &mut C) -> Result<(), ApplicationError> {
    for signature in FUNCTIONS {
        let row = client.query_opt("SELECT p.prosrc,p.provolatile::text,p.prorettype::regtype::text,
            p.prosecdef,p.proconfig,p.proisstrict,p.proleakproof,l.lanname,n.oid=c.relnamespace,
            quote_ident(n.nspname),p.prokind::text,p.proparallel::text
            FROM pg_proc p JOIN pg_language l ON l.oid=p.prolang JOIN pg_namespace n ON n.oid=p.pronamespace
            JOIN pg_class c ON c.oid='deadline_source_events'::regclass
            WHERE p.oid=to_regprocedure($1)", &[&signature]).map_err(port)?.ok_or_else(incomplete)?;
