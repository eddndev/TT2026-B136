use crate::{
    procedural_facts::{FactDeclaration, FactEvidence, FactResolutionRef},
...
pub(super) fn resolution(bytes: &mut Vec<u8>, value: FactResolutionRef) {
    bytes.extend_from_slice(value.id.as_uuid().as_bytes());
    bytes.extend_from_slice(&value.revision.get().to_be_bytes());
}
pub(super) fn evidence(bytes: &mut Vec<u8>, value: &FactEvidence) {
